...
// permission loop inside its own native sandbox + GUI, so AgentHalo never issues a
...
function showPermissionBubble(permEntry) {
  ...
  try {
    ...
    repositionDependentBubbles();
    // macOS: defer full visibility restoration to avoid activating AgentHalo
...
// an opencode-family AskUserQuestion cannot be answered in AgentHalo
...
// a blank "Tool input hidden by AgentHalo" card would let the user approve a black
...
function handleBubbleExpanded(event, expanded) {
  ...
  // Codex request_user_input is intentionally read-only in AgentHalo because the
...
function cleanup() {
  ...
  for (const perm of [...pendingPermissions]) {
    ...
    if (isPassiveNotifyEntry(perm)) dismissPassiveNotify(perm, "AgentHalo is quitting");
    else dismissInteractivePermissionWithoutDecision(perm, "AgentHalo is quitting");
  }
  destroyQueueWindow({ resetEpisode: true });
