...
// exists" only proves AgentHalo ran, so they are excluded from local detection.
//
// #895: this used to be the whole default-integration list, on the assumption
// that AgentHalo creates both ~/.claude and ~/.codex. Only the first is true —
...
function detectClawdIntegration(descriptor, paths, options) {
  ...
  if (descriptor.agentId === "deepseek-harness") {
    ...
    return health.status === "healthy"
      ? {
        ...
        detail: `${health.profileDir} contains the verified AgentHalo bridge`,
  ...
  if (descriptor.agentId === "pi") {
    ...
    return fileExists(fsImpl, markerPath)
      ...
      : { detected: false, reason: "not-found", detail: "No AgentHalo-managed Pi extension marker found" };
  }
  if (descriptor.agentId === "hermes") {
    ...
    return found
      ? { detected: true, reason: "managed-files", detail: `${paths.configPath} contains AgentHalo plugin files`, paths: { pluginDir: paths.configPath } }
      : { detected: false, reason: "not-found", detail: "No AgentHalo-managed Hermes plugin files found" };
  }
  ...
  return {
    detected: false,
    reason: "not-found",
    detail: `No ${descriptor.marker || "AgentHalo"} marker found`,
  };
}
...
async function refreshWslDetection(options = {}) {
  ...
  try {
    ...
    for (const distro of distros) {
      ...
      // DEPFILE/DEPREG lines report the distro's AgentHalo hook deployment
      // state (see below).
      ...
      for (let i = 0; i < checks.length; i++) {
        ...
        batchLines.push(
          ...
          + `\\( -path '*/plugins/agenthalo/plugin.yaml' -o -path '*/plugins/agenthalo/__init__.py' \\) `
      ...
      if (!batchResult || batchResult.error || batchResult.code !== 0) {
        console.warn("AgentHalo: WSL batch dir check failed in", distro.name, "—",
      ...
      if (markerError) {
        console.warn("AgentHalo: WSL batch marker output was incomplete or ambiguous in", distro.name);
        keepPreviousEntries(distro.name);
        continue;
      }
    ...
    console.warn("AgentHalo: WSL detection scan failed:", err && err.message ? err.message : err);
    _cachedDetected = true;
