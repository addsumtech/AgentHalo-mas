...
function createRenderProcessGoneReloadGuard(options = {}) {
  ...
  return function shouldReloadAfterRenderProcessGone(crashKey, details) {
    ...
    if (NON_RELOADABLE_RENDER_GONE_REASONS.has(reason)) {
      log(`AgentHalo: not reloading ${key} after render-process-gone (${reason})`);
      return false;
    }
    ...
    if (recent.length >= crashReloadLimit) {
      log(`AgentHalo: stopped reloading ${key} after ${recent.length} crashes in ${crashReloadWindowMs}ms`);
...
function createPetWindowRuntime(options = {}) {
  ...
  function logEdgeOnce(reason, detail) {
    ...
    edgeLog(`AgentHalo: edge-${reason} ${detail}`);
  }
  ...
  function updateObservedClampInset(displayId, edge, inset) {
    ...
    edgeLog(`AgentHalo: inset-drift display=${displayId} edge=${edge} old=${Number.isFinite(prev) ? prev : 0} new=${inset}`);
  }
  ...
  function logReconcileEvent(windowLabel, predicted, actual, action, settleState, sinceWriteMs, extra) {
    ...
    const delta = predicted && actual
      ...
      : "n/a";
    edgeLog(
      `AgentHalo: edge-reconcile window=${windowLabel} predicted=${rectStr(predicted)} actual=${rectStr(actual)} `
  ...
  function handleHitExternalMoveCandidate(actualHit) {
    edgeLog(
      `AgentHalo: edge-hit-external-move-candidate action=log-only `
  ...
  function createRenderWindow(optionsArg = {}) {
    ...
    if (isLinux) {
      ...
      renderWin.on("unresponsive", () => {
        ...
        console.warn("AgentHalo: renderer unresponsive — reloading");
    ...
    if (isWin) {
      ...
      const flushForSessionEnd = () => {
        ...
        try {
          ...
          console.warn("AgentHalo: failed to persist prefs during Windows session end:", err && err.message);
        }
      };
  ...
  function createHitWindow(optionsArg = {}) {
    ...
    const hitWin = new BrowserWindow({
      ...
      // activate AgentHalo on a fullscreen click/drag. If that controller could
