...
// App scripting can block on the macOS Automation consent
// dialog on first use; killing it early dismisses the dialog before the user
// can answer (#465), so those scripts get a human-scale timeout.
...
function focusAppleTerminalTab(request, callback) {
  const candidates = (request.pidChain || []).filter(pid => pid !== request.sourcePid).slice(0, 8);
  if (!candidates.length) {
    logFocusResult("branch=terminal-tab reason=no-pid-chain");
    callback(false);
    return;
  }
  execFile("ps", ["-o", "pid=,tty=", "-p", candidates.join(",")], { encoding: "utf8", timeout: 1000 }, (_err, stdout) => {
    ...
    for (const line of String(stdout || "").split("\n")) {
      const match = line.match(/^\s*(\d+)\s+(?:\/dev\/)?(tty[a-zA-Z0-9]+)\s*$/);
      if (match) ttyByPid.set(Number(match[1]), `/dev/${match[2]}`);
    }
    const tty = candidates.map(pid => ttyByPid.get(pid)).find(Boolean);
    if (!tty) {
      logFocusResult("branch=terminal-tab reason=no-tty");
      callback(false);
      return;
    }
    const script = `
          ...
          repeat with w in windows
            repeat with t in tabs of w
              if tty of t is targetTty then
                ...
                set selected tab of window id targetWindowId to t
                set miniaturized of window id targetWindowId to false
                set index of window id targetWindowId to 1
                activate
                if tty of selected tab of front window is targetTty then return "ok-tty"
                return "unconfirmed-tty"
              end if
            end repeat
          end repeat
          return "miss-tty"
    ...
    execFile("osascript", ["-e", script, tty], { encoding: "utf8", timeout: MAC_FOCUS_CONSENT_TIMEOUT_MS }, (err, out, stderr) => {
      const detail = String(stderr || (err && err.message) || "");
      const reason = err
        ? (detail.includes("-1743") ? "automation-denied" : `osascript-failed:${safeLogValue(err.signal || err.code || "error")}`)
        : String(out || "").trim() || "miss-tty";
      logFocusResult(`branch=terminal-tab reason=${safeLogValue(reason)}`);
      callback(!err && reason === "ok-tty");
    });
  });
}
  ...
  if (isMac) {
    ...
    resolveMacAppBundle(pidCandidates, (bundlePath) => {
      ...
      if (bundlePath === "/System/Applications/Utilities/Terminal.app"
        ...
        // Keep the macOS focus queue held until tab selection AND activation
        // finish, so a later click cannot be overwritten by an older request.
        focusAppleTerminalTab(request, (focused) => {
          if (!focused) openBundle();
