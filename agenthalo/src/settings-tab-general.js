...
(function initSettingsTabGeneral(root) {
  ...
  function render(parent) {
    ...
    parent.appendChild(helpers.buildSection(t("sectionAppearance"), [
      buildLanguageRow(), buildUiColorThemeRow(), buildSizeSliderRow(), buildTextScaleRow(),
    ]));
    // Sound and roaming shared one card titled "Sound and companionship", which
    // named one concrete thing plus a vague catch-all and put an expandable
    // roam row next to two plain switches. They are separate concerns: sound is
    // how the companion gets your attention, roaming is what it does on its own.
    parent.appendChild(helpers.buildSection(t("sectionAlerts"), [buildSoundGroup()]));
    parent.appendChild(helpers.buildSection(t("sectionBehavior"), [buildFreeRoamGroup()]));
    ...
    links.className = "settings-secondary-actions";
    for (const [id, label] of [["shortcuts", "sidebarShortcuts"], ["recap", "sidebarRecap"]]) {
      const button = document.createElement("button");
      button.type = "button";
      button.className = "settings-footer-link";
      button.textContent = t(label);
      button.addEventListener("click", () => ops.selectTab(id));
  ...
  function buildUiColorThemeRow() {
    const row = document.createElement("div");
    row.className = "row ui-color-theme-row";
    const text = document.createElement("div");
    text.className = "row-text";
    const label = document.createElement("span");
    label.className = "row-label";
    label.textContent = t("rowUiColorTheme");
    const desc = document.createElement("span");
    desc.className = "row-desc";
    desc.textContent = t("rowUiColorThemeDesc");
    text.append(label, desc);
    const catalog = root.AgentHaloColorThemes;
    const picker = helpers.buildSegmentedRadio({
      value: (state.snapshot && state.snapshot.uiColorTheme) || catalog.DEFAULT_THEME,
      ariaLabel: t("rowUiColorTheme"),
      className: "ui-color-theme-picker",
      options: catalog.THEMES.map((theme) => ({ value: theme.id, label: t(theme.key) })),
      async onChange(next) {
        try {
          const result = await window.settingsAPI.update("uiColorTheme", next);
          if (result && result.status === "ok") return true;
          ops.showToast(t("toastSaveFailed") + ((result && result.message) || "unknown error"), { error: true });
        } catch (err) {
          ops.showToast(t("toastSaveFailed") + err.message, { error: true });
        }
        return false;
      },
    });
    for (const button of picker.element.querySelectorAll("button")) {
      const theme = catalog.getTheme(button.dataset.value);
      button.style.setProperty("--preview-bg", theme.bg);
      button.style.setProperty("--preview-sidebar", theme.sidebar);
      button.style.setProperty("--preview-accent", theme.accent);
      const swatch = document.createElement("span");
      swatch.className = "ui-color-theme-swatch";
      swatch.setAttribute("aria-hidden", "true");
      button.insertBefore(swatch, button.firstChild);
    }
    row.append(text, picker.element);
    state.mountedControls.uiColorTheme = picker;
    return row;
  }
  ...
  function buildSoundGroup() {
    const group = document.createElement("div");
    group.className = "sound-settings-group";
    const row = document.createElement("div");
    row.className = "row sound-toggle-row";
    const text = document.createElement("div");
    text.className = "row-text";
    const label = document.createElement("span");
    label.className = "row-label";
    label.textContent = t("rowSound");
    const desc = document.createElement("span");
    desc.className = "row-desc";
    desc.textContent = t("rowSoundDesc");
    text.append(label, desc);
    ...
    state.mountedControls.generalSwitches.set("soundMuted", {
      element: summaryControl.headerSwitch, invert: true, row, text, extraElement: null,
    });
    row.append(text, summaryControl.element);
    group.append(row, buildVolumeSliderRow());
    return group;
  }
  ...
  function buildVolumeSliderRow() {
    ...
    const previewPlayback = window.AgentHaloSoundPlayback.createSoundPlayback(
      () => window.settingsAPI.isSystemAudioMuted()
    );
    ...
    function playPreview(vol) {
      ...
      previewPlayback.play(previewAudio).catch(() => {});
    }
  ...
  function buildTextScaleRow() {
    const row = document.createElement("div");
    row.className = "row text-scale-row";
    row.innerHTML =
      `<div class="row-text">` +
        `<span class="row-label"></span>` +
        `<span class="row-desc"></span>` +
  ...
  function buildSizeSliderRow() {
    const row = document.createElement("div");
    row.className = "row size-slider-row";
    row.innerHTML =
      `<div class="row-text">` +
        `<span class="row-label"></span>` +
        `<span class="row-desc"></span>` +
    ...
    row.querySelector(".row-desc").remove();
  ...
  function syncMacAppPresenceSwitchesDisabled() {
    ...
    return setGeneralSwitchDisabled("showTray", false)
      && setGeneralSwitchDisabled("showDock", false);
  }
  ...
  function patchInPlace(changes) {
    ...
    if (keys.includes("uiColorTheme")) {
      const picker = state.mountedControls.uiColorTheme;
      if (!picker || !document.body.contains(picker.element)) return false;
      picker.setValue(state.snapshot.uiColorTheme);
    }
    ...
    for (const key of keys) {
      if (key === "size" || key === "uiColorTheme") continue;
