...
const SLACK_LOCALES = Object.freeze({
  en: {
    ...
    testBody: "This is a test message from AgentHalo. If you can read this, notifications are working.",
  ...
  zh: {
    ...
    testBody: "这是来自 AgentHalo 的测试消息。如果你能看到它，说明通知已正常工作。",
    wrapStatus: (status) => `（${status}）`,
  },
  "zh-TW": {
    session: "工作階段",
    ...
    testBody: "這是來自 AgentHalo 的測試訊息。如果你能看到它，代表通知已正常運作。",
    wrapStatus: (status) => `（${status}）`,
  },
  ko: {
    session: "세션",
    ...
    testBody: "AgentHalo에서 보낸 테스트 메시지입니다. 이 메시지가 보이면 알림이 정상 작동합니다.",
    wrapStatus: (status) => `(${status})`,
  },
  ja: {
    session: "セッション",
    ...
    testBody: "AgentHalo からのテストメッセージです。これが表示されていれば通知は正常に動作しています。",
    wrapStatus: (status) => `（${status}）`,
  },
  "pt-BR": {
    session: "sessão",
    ...
    testBody: "Esta é uma mensagem de teste do AgentHalo. Se você consegue ler isto, as notificações estão funcionando.",
    wrapStatus: (status) => `(${status})`,
  },
  es: {
    session: "sesión",
    ...
    testBody: "Este es un mensaje de prueba de AgentHalo. Si puedes leerlo, las notificaciones funcionan.",
    wrapStatus: (status) => `(${status})`,
  },
});
