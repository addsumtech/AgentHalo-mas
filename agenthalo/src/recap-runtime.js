...
function createRecapRuntime(options = {}) {
  ...
  function notifyChanged() {
    if (!onChanged) return;
    try { onChanged(); }
    catch (err) { warn("AgentHalo: recap change notification failed", err); }
  }
  ...
  function initialize() {
    ...
    try {
      ...
      if (!isTransientRecapPrivateAclError(err) || storageRetryAttempt === 0) {
        warn("AgentHalo: local recap storage is unavailable; recording is paused", unavailableCode);
      }
      ...
      .then(async ({ dates, records, truncated }) => {
        ...
        if (truncated) {
          ...
          try { aggregate.flush(); } catch (err) {
            warn("AgentHalo: recap aggregate privacy migration flush failed", err && err.code ? err.code : "storage-error");
          }
          warn("AgentHalo: recap journal reconciliation skipped because retained history exceeded its safety bound");
          return;
        }
        ...
        try {
          ...
          warn("AgentHalo: recap aggregate reconciliation flush failed", err && err.code ? err.code : "storage-error");
        }
        hydrationRebuilding = false;
      })
      .catch((err) => {
        ...
        try { aggregate.flush(); } catch (flushErr) {
          warn("AgentHalo: recap aggregate privacy migration flush failed", flushErr && flushErr.code ? flushErr.code : "storage-error");
        }
        warn("AgentHalo: recap journal reconciliation failed", err && err.code ? err.code : "storage-error");
      });
  ...
  function scheduleMidnight() {
    ...
    midnightTimer = setTimer(() => {
      ...
      try {
        ...
        warn("AgentHalo: recap midnight rollover failed", err);
      }
      scheduleMidnight();
  ...
  function handleSuspend() {
    ...
    try {
      ...
      warn("AgentHalo: recap suspend checkpoint failed", err && err.code ? err.code : "storage-error");
    }
  }
  ...
  function handleResume() {
    ...
    try {
      ...
      warn("AgentHalo: recap resume checkpoint failed", err && err.code ? err.code : "storage-error");
    }
  }
  ...
  function clear() {
    ...
    try { store.clear(); } catch (err) {
      ...
      enabled = false;
      warn("AgentHalo: local recap clear failed", unavailableCode);
      scheduleStorageRetry(err);
      return false;
    }
    if (!initialize()) return false;
  ...
  function dispose() {
    ...
    if (initialized) {
      try {
        ...
        warn("AgentHalo: local recap shutdown flush failed", err && err.code ? err.code : "storage-error");
      }
      try { coverage.resetMemory(); } catch {}
    }
    started = false;
