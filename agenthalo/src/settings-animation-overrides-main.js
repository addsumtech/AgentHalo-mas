...
const ANIMATION_OVERRIDES_EXPORT_DIALOG_STRINGS = {
  en: {
    ...
    jsonFilter: "AgentHalo Animation Overrides",
  ...
  "zh-TW": {
    ...
    defaultName: (ts) => `clawd-animation-overrides-${ts}.json`,
    jsonFilter: "AgentHalo 動畫與音效自訂設定",
  ...
  ko: {
    ...
    defaultName: (ts) => `clawd-animation-overrides-${ts}.json`,
    jsonFilter: "AgentHalo 애니메이션 덮어쓰기",
  ...
  ja: {
    ...
    defaultName: (ts) => `clawd-animation-overrides-${ts}.json`,
    jsonFilter: "AgentHalo アニメーション差し替え",
  ...
  "pt-BR": {
    ...
    defaultName: (ts) => `clawd-animation-overrides-${ts}.json`,
    jsonFilter: "Personalizações de animação do AgentHalo",
  ...
  es: {
    ...
    defaultName: (ts) => `clawd-animation-overrides-${ts}.json`,
    jsonFilter: "Personalizaciones de animación de AgentHalo",
...
function createSettingsAnimationOverridesMain(options = {}) {
  ...
  function runPendingPostReloadTasks() {
    ...
    for (const task of tasks) {
      try { task(); } catch (err) { console.warn("AgentHalo: post-reload task threw:", err && err.message); }
    }
  }
  ...
  async function runAnimationPreviewPosterJob(job) {
    try {
      ...
      if (err && err.code === "ANIMATION_PREVIEW_POSTER_TIMEOUT") {
        console.warn("AgentHalo: animation preview poster capture timed out:", message);
      } else {
        console.warn("AgentHalo: failed to capture animation preview poster:", message);
      }
    } finally {
  ...
  async function exportAnimationOverrides(event) {
    ...
    try {
      ...
      console.warn("AgentHalo: export-animation-overrides failed:", err && err.message);
  ...
  async function importAnimationOverrides(event) {
    ...
    try {
      ...
      console.warn("AgentHalo: import-animation-overrides dialog failed:", err && err.message);
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return { status: "error", message: "file is not a AgentHalo animation overrides export" };
    }
