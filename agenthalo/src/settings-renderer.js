...
const SECONDARY_TABS = { animOverrides: "theme", shortcuts: "general", recap: "general" };
...
function renderSidebar() {
  ...
  brand.innerHTML = '<img src="../assets/icon.png" alt=""><span>AgentHalo</span>';
  ...
  for (const tab of SIDEBAR_TABS) {
    ...
    if (tab.id === (SECONDARY_TABS[core.state.activeTab] || core.state.activeTab)) {
...
function renderContent() {
  ...
  content.innerHTML = "";
  const parentTab = SECONDARY_TABS[core.state.activeTab];
  if (parentTab) {
    const back = document.createElement("button");
    back.type = "button";
    back.className = "soft-btn settings-back";
    back.textContent = "← " + core.helpers.t(SIDEBAR_TABS.find(tab => tab.id === parentTab).labelKey);
    back.addEventListener("click", () => core.ops.selectTab(parentTab));
