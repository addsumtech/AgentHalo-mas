...
const updateRegistry = {
  ...
  uiColorTheme: requireEnum("uiColorTheme", require("./ui-color-theme").THEMES.map((theme) => theme.id)),
  ...
  showDock: requireBoolean("showDock"),
...
function openTutorial(_payload, deps = {}) {
  if (typeof deps.openTutorial !== "function") {
    return { status: "error", message: "Tutorial is not available." };
  }
  try { deps.openTutorial(); }
  catch (err) {
    return { status: "error", message: (err && err.message) || "Could not open the welcome guide." };
  }
  return { status: "ok" };
}
