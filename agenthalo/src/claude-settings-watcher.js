 ...
 * TODO: Decide whether non-AgentHalo HTTP hooks should contribute to third-party shrink detection.
...
function createClaudeSettingsWatcher(ctx = {}) {
  ...
  async function handleReport(report, raw, reason, tokenAtStart) {
    ...
    if (report.status === "source-script-missing") {
      ...
      if (!sourceMissingLogged) {
        console.warn("AgentHalo: the current Claude hook source script is missing — reinstall or re-extract AgentHalo to restore automatic hook repair");
    ...
    if (isSuspiciousShrink(lastTrustedSnapshot, currentSnapshot, suspiciousShrinkRatio, suspiciousKeyLossThreshold)) {
      ...
      if (!shrinkNotified) {
        console.warn("AgentHalo: settings.json shrank suspiciously — skipping auto-repair to preserve third-party hooks");
  ...
  function start() {
    ...
    try {
      ...
      console.warn("AgentHalo: could not seed settings baseline:", err.message);
    }
    ...
    try {
      ...
      if (settingsWatcher && typeof settingsWatcher.on === "function") {
        settingsWatcher.on("error", (err) => {
          console.warn("AgentHalo: settings watcher error:", err.message);
        });
      }
    } catch (err) {
      console.warn("AgentHalo: failed to watch settings directory:", err.message);
      settingsWatcher = null;
      return false;
    }
