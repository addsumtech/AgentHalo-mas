...
function createHolidayAccessoryRuntime(options = {}) {
  ...
  function refresh({ force = false } = {}) {
    ...
    try { resolved = resolveDisplay(); } catch (err) {
      try { logWarn("AgentHalo: holiday accessory refresh failed:", err && err.message); } catch {}
      return false;
    }
    ...
    if (force || resolved.key !== lastDeliveredKey) {
      try {
        ...
        try { logWarn("AgentHalo: holiday accessory delivery failed:", err && err.message); } catch {}
        return false;
      }
    ...
    if (force || resolved.key !== lastAppliedKey) {
      try {
        ...
        try { logWarn("AgentHalo: holiday accessory geometry apply failed:", err && err.message); } catch {}
        return false;
      }
    }
    return changed;
