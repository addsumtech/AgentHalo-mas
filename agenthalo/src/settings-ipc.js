...
function mapAgentMetadata(agent) {
  const metadata = {
    ...
    // their parent dirs are not trustworthy evidence (AgentHalo's own Claude sync
...
function registerSettingsIpc(options = {}) {
  ...
  const saveFeishuApproverByEmail = options.saveFeishuApproverByEmail
    || (() => ({ status: "error", message: "Remote approvals are no longer available" }));
  ...
  function handle(channel, listener) {
    if (["settings:mobile-connection-info", "settings:regenerate-mobile-token", "settings:reset-mobile-access"].includes(channel)) return;
  ...
  let webBridgeCheckPending = null;
  handle("settings:check-web-bridge", (event) => {
    const rejected = rejectUntrustedSettingsEvent(event);
    if (rejected) return rejected;
    if (!webBridgeCheckPending) {
      webBridgeCheckPending = require("./web-bridge-check-process").checkWebBridgeInProcess()
        .then((report) => {
          const {
          ...
          const applications = (Array.isArray(snapshot.customApplications) ? snapshot.customApplications : [])
            .filter(isWebBridgeApplication);
          const inspected = inspectWebBridgeInstall();
          const recentEvents = applications.flatMap((application) => (
            getRecentHookEvents({ agentId: application.id })
          ...
          return {
            ...
            preparedVersion: inspected.version,
            storeUrl: chromeWebStoreUrl(),
            ...summarizeWebBridgeConnection({
              filesReady: inspected.filesReady,
              applications,
              installations: report && report.installations,
              recentEvents,
            }),
          };
        })
        .finally(() => { webBridgeCheckPending = null; });
    }
    return webBridgeCheckPending;
  });
  ...
  handle("settings:command", async (event, payload) => {
    ...
    if (require("./local-product-policy").isRetiredSetting(payload.action)) {
      return { status: "error", message: "This remote feature is no longer available" };
    }
  ...
  handle("settings:list-themes", () => {
    try {
      const activeTheme = getActiveTheme();
      const activeId = activeTheme ? activeTheme._id : "halo";
      ...
      console.warn("AgentHalo: settings:list-themes failed:", err && err.message);
      return [];
    }
  });
  ...
  handle("settings:import-user-theme-zip", async (event) => {
    ...
    try {
      result = await dialog.showOpenDialog(getDialogParent(event), {
        properties: ["openFile"],
        filters: [{ name: "AgentHalo theme zip", extensions: ["zip"] }],
      });
    } catch (err) {
  ...
  handle("settings:confirm-remove-theme", async (event, themeId) => {
    ...
    const lang = getLang();
    const displayName = localizeField(meta && meta.name, lang) || themeId;
    ...
    try {
      ...
      console.warn("AgentHalo: confirm-remove-theme dialog failed:", err && err.message);
  ...
  handle("settings:list-agents", () => {
    try {
      ...
      console.warn("AgentHalo: settings:list-agents failed:", err && err.message);
      return [];
    }
  });
  ...
  handle("settings:get-about-info", () => {
    ...
    return {
      ...
      upstreamCopyright: "\u00a9 2026 Ruller_Lulu and contributors",
