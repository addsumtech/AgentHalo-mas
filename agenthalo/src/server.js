...
const { createCursorSubagentWindows } = require("./cursor-subagent-window");
const {
...
// (whose contents depend on whether AgentHalo happens to be running right now).
...
const cursorSubagentWindows = createCursorSubagentWindows();
...
function getRuntimeStatus() {
  ...
  // as "AgentHalo offline" to every hook even while this server is happily
  ...
  return async () => {
    ...
    if (claudeHookSourceMissing({ requireAutoStart: !!meta.autoStart })) {
      return {
        ...
        message: "Claude hook source script is missing; reinstall or re-extract AgentHalo",
      };
    }
    ...
    if (added > 0 || updated > 0 || removed > 0) {
      console.log(`AgentHalo: synced hooks (added ${added}, updated ${updated}, removed ${removed}) [${meta.source || "unspecified"}]`);
    }
  ...
  return claudeHookOperations.enqueue({ source, automatic: false }, async () => {
    ...
    if (result.skippedExisting) {
      return {
        ...
        message: "Claude Code already has a custom statusline; AgentHalo left it unchanged",
      };
    }
...
function repairRuntimeStatus() {
  ...
  return {
    status: "error",
    message: "Local server is not listening; restart AgentHalo",
  };
}
...
function routeHttpRequest(req, res, remoteProfile = null) {
    ...
    if (req.method === "GET" && req.url === "/state") {
      ...
      const applications = typeof ctx.getCustomApplications === "function"
        ? ctx.getCustomApplications()
        : [];
      sendWebBridgeDiscoveryResponse(res, { applications });
  ...
  return new Promise((resolve) => {
    ...
    httpServer.on("listening", () => {
      ...
      try {
        ...
        console.warn("Failed to write the AgentHalo runtime file:", (err && err.message) || err);
      }
      if (!runtimeWritten) {
        ...
        console.warn(
          `AgentHalo runtime file was not written (${runtimeConfigFilePath()}) — `
          + "hook process metadata will be omitted until this is repaired (see Doctor → Local server)"
        );
      }
      console.log(`AgentHalo state server listening on 127.0.0.1:${activeServerPort}`);
