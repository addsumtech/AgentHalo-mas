...
function _scanThemesDir(dir, builtin, themes, seen) {
  try {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (cfg && cfg._scaffoldOnly === true) continue;
 ...
 *
 * Strict mode throws on missing/invalid; lenient uses the bundled default.
...
function loadTheme(themeId, opts = {}) {
  ...
  if (!raw) {
    ...
    if (themeId !== defaultThemeId) return loadTheme(defaultThemeId);
    throw new Error(`Default theme '${defaultThemeId}' not found`);
  }
