...
window.hitAPI.onCancelReaction(() => {
  ...
  clearTimeout(reactionGateTimer);
  reactionGateTimer = null;
  isReacting = false;
...
function playReaction(svg, duration) {
  ...
  // Tracked, so cancelReaction can clear it and so a new reaction supersedes
  // the previous gate. Untracked, a stale timer reopened the click gate part
  // way through the next reaction and let a third click stack on top of it.
  clearTimeout(reactionGateTimer);
  reactionGateTimer = setTimeout(() => { isReacting = false; }, duration);
}
...
if (!isMac) {
  ...
  // fallback chain: double (AgentHalo) → clickLeft/clickRight poke (Calico) →
