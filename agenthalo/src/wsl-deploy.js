...
function collectHookFiles(hooksDir) {
  ...
  try {
    ...
    console.warn("AgentHalo: collectHookFiles failed:", err && err.message ? err.message : err);
    throw err;
  }
  return files;
}
  ...
  try {
    ...
    if (!copied.ok) {
      ...
      if (!parsed.ok || !runResult || runResult.code !== 0 || parsed.result.status === "error") {
        ...
        else if (connectivity.reachable === false) emit("verify-connectivity", "warn", "AgentHalo HTTP server unreachable from WSL (NAT networking?)");
