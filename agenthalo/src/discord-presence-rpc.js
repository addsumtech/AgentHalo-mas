...
// AgentHalo sprite per resolved image fallback state (see resolvePresenceState).
// Mirror fallback path: used when no on-screen visual is known yet (bridge just
// started), or the current theme/svg cannot use the exact AgentHalo mapping.
...
// "interrupted" for the image fallback. In the normal AgentHalo bridge path,
...
function agentLabel(agentId) {
  ...
  return (agent && agent.name) || "AgentHalo";
}
...
function buildPresencePayload(session, cfg = {}, visual = null, runtime = {}) {
  ...
  const activity = {
    ...
    state: label,
    assets: { large_image: imageUrl, large_text: "AgentHalo" },
  };
