...
function createRecapCoverage(options = {}) {
  ...
  function readOpenHeartbeat() {
    ...
    if (!saved || saved.schemaVersion !== COVERAGE_SCHEMA_VERSION) {
      ...
      warn("AgentHalo: discarded invalid recap coverage heartbeat");
      return null;
    }
  ...
  function load() {
    ...
    for (const name of names) {
      ...
      if (
        ...
        warn("AgentHalo: quarantined invalid recap coverage file");
        continue;
      }
  ...
  function scheduleHeartbeat() {
    ...
    heartbeatTimer = setTimer(() => {
      heartbeatTimer = null;
      try { tick(now()); } catch (err) { warn("AgentHalo: recap coverage heartbeat failed", err); }
      scheduleHeartbeat();
    }, heartbeatMs);
