...
const HUD_HEIGHT = HUD_ROW_HEIGHT + HUD_TOOLBAR_HEIGHT + HUD_BORDER_Y;
...
function computeHudLayout(snapshot, options = {}) {
  ...
  const visible = ordered.concat(missing).filter(isHudSession).sort((a, b) =>
    (HUD_BADGE_ORDER[a.badge] ?? 1) - (HUD_BADGE_ORDER[b.badge] ?? 1)
  );
  return { expanded: visible, folded: [], rowCount: visible.length };
}
...
function computeHudHeight(rowCount) {
  ...
  return rowCount * HUD_ROW_HEIGHT + HUD_TOOLBAR_HEIGHT + HUD_BORDER_Y;
}
...
function computeSessionHudBounds({ hitRect, anchorRect, workArea, width = HUD_WIDTH, height = HUD_HEIGHT, scale = 1, widthScale = scale }) {
  ...
  // Keep every task in the DOM; cap only the viewport so long lists scroll.
  const availableHeight = Math.max(HUD_ROW_HEIGHT * s,
    workArea.height - EDGE_MARGIN * 2 * s - (HUD_WINDOW_SHELL.top + HUD_WINDOW_SHELL.bottom) * s);
  const dipHeight = Math.min(Math.ceil(height * s), Math.floor(availableHeight));
  const shell = {
...
module.exports = function initSessionHud(ctx) {
  ...
  function computeBounds(snapshot, scale = getTextScale()) {
    ...
    const widthScale = getHudWidthScale(scale);
    const computed = computeSessionHudBounds({ hitRect, anchorRect, workArea, width, height, scale, widthScale });
    lastHudHeight = computed ? computed.contentBounds.height / scale : height;
    return computed;
  }
