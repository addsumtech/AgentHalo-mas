...
// here — the trust boundary is "machines the user deployed AgentHalo hooks to",
...
function createAccountQuotaStore(options = {}) {
  ...
  function persistNow() {
    ...
    try {
      ...
      logWarn("AgentHalo: account-quota persist failed:", err && err.message);
      return false;
    }
  }
  ...
  function updateDetailed(source, quotas = {}) {
    ...
    if (!record && sources.size >= MAX_SOURCES) {
      logWarn("AgentHalo: account-quota source cap reached, dropping report from:", key || "(local)");
