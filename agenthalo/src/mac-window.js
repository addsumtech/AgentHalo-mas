...
function delegateWindowToStationarySpace(nsWindow) {
  try {
    ...
    if (!warnedSkyLightFailure) {
      console.warn("AgentHalo: failed to move macOS window into stationary SkyLight space:", err.message);
...
function deDelegateWindowFromStationarySpace(browserWindow, level = 0) {
  ...
  try {
    ...
    return runDeDelegateTransaction({
      ...
      onFailure: (err) => {
        if (!warnedSkyLightFailure) {
          console.warn("AgentHalo: failed to de-delegate window from stationary SkyLight space:", err.message);
          warnedSkyLightFailure = true;
        }
      },
    });
