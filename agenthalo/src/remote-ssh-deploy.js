...
async function acquireDeployLock({
  ...
  return {
    ...
    message: reason === "lock_busy"
      ? "Another AgentHalo deployment owns this remote runtime; try again later."
...
async function runOwnershipPreflight({
  ...
  return {
    ...
    message: reason === "local_clawd_conflict"
      ? "A live AgentHalo desktop is using this remote account's default configuration. Deployment is blocked."
      : (reason === "ownership_conflict"
        ? "This remote runtime is owned by another AgentHalo installation or profile."
...
async function secureDeploy({
  ...
  try {
    ...
    if (componentPresence.identity !== true
      ...
      return fail(
        "preflight",
        "This remote runtime contains a legacy AgentHalo deployment with no verifiable owner. Confirm migration before changing it.",
