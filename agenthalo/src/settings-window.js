      ...
      if (result && typeof result.then === "function") {
        ...
        Promise.resolve(result).then(
          (response) => {
            ...
            console.warn("AgentHalo: failed to persist Settings window bounds:", response.message);
          },
          (err) => {
            if (sameBounds(lastSavedBounds, attemptedBounds)) lastSavedBounds = null;
            console.warn("AgentHalo: failed to persist Settings window bounds:", err && err.message);
          },
        );
      } else if (result && result.status === "error") {
        console.warn("AgentHalo: failed to persist Settings window bounds:", result.message);
        return false;
      }
