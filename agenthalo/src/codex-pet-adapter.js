...
function generateWrapperSvg({ rowKey, mode, spritesheetHref, atlas = ATLAS }) {
  ...
  return [
    `<!-- Generated by AgentHalo codex-pet-adapter v${ADAPTER_VERSION}: ${row.key} ${mode}. -->`,
...
function generateUniversalWrapperSvg({ initialFilename, spritesheetHref, atlas = ATLAS }) {
  ...
  return [
    `<!-- Generated by AgentHalo codex-pet-adapter v${ADAPTER_VERSION}: universal visual bridge (${initialSpec.visual}). -->`,
...
function validatePngAtlasAlpha(idatChunks, info, atlas) {
  ...
  // containing the 16 clockwise gaze directions. AgentHalo does not consume those
