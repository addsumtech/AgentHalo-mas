...
function createMacDockVisibilityCoordinator(options = {}) {
  ...
  function installDockIcon() {
    ...
    try {
      ...
      warn("AgentHalo: failed to install macOS Dock icon:", err);
    }
  }
  ...
  function resolveInstallDockIcon() {
    try {
      ...
      warn("AgentHalo: failed to resolve macOS Dock icon policy:", err);
      return false;
    }
  }
  ...
  function reapplyVisibility() {
    try {
      ...
      warn("AgentHalo: failed to reapply macOS window visibility after Dock transition:", err);
    }
  }
  ...
  function captureFocusedSettingsWindow() {
    try {
      ...
      warn("AgentHalo: failed to inspect Settings focus before Dock transition:", err);
      return null;
    }
  }
  ...
  function restoreSettingsFocus(win) {
    ...
    try {
      ...
      warn("AgentHalo: failed to preserve Settings focus while hiding Dock:", err);
    }
  }
  ...
  function applyNativeVisibility(visible) {
    ...
    if (!app || typeof app.setActivationPolicy !== "function") {
      warn("AgentHalo: macOS activation policy API is unavailable");
      reapplyVisibility();
      return;
    }
      ...
      .then(() => {
        while (needsDrain && !disposed) {
          ...
          try {
            ...
            warn("AgentHalo: macOS Dock transition failed:", err);
          }
