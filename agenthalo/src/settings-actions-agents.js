...
async function authorizeAgentConfigDir(payload, deps = {}) {
  const agentId = typeof payload === "string" ? payload : payload && payload.agentId;
  const idCheck = requireString("authorizeAgentConfigDir.agentId")(agentId);
  if (idCheck.status !== "ok") return idCheck;
  if (!deps || typeof deps.authorizeAgentConfigDir !== "function") {
    return { status: "error", message: "authorizeAgentConfigDir requires authorizeAgentConfigDir dep" };
  }
  try {
    const result = await deps.authorizeAgentConfigDir(agentId, {
      force: !!(payload && typeof payload === "object" && payload.force === true),
    });
    ...
    return {
      status: "error",
      message: `authorizeAgentConfigDir: ${err && err.message}`,
    };
  }
}

function listAuthorizedConfigDirs(_payload, deps = {}) {
  if (!deps || typeof deps.listAuthorizedConfigDirs !== "function") {
    return { status: "ok", dirs: {} };
  }
  try {
    return { status: "ok", dirs: deps.listAuthorizedConfigDirs() || {} };
  } catch (err) {
    return { status: "error", message: `listAuthorizedConfigDirs: ${err && err.message}` };
  }
}
...
async function installAgentIntegration(payload, deps = {}) {
  ...
  if (typeof deps.authorizeAgentConfigDir === "function") {
    try {
      const auth = await deps.authorizeAgentConfigDir(agentId, { force: false });
...
authorizeAgentConfigDir.lockKey = "agentIntegration";
...
function ensureWebBridge(_payload, deps = {}) {
  const ensured = ensureWebBridgeApplications({
    fs: deps.fs,
    path: deps.path,
    home: deps.home,
  ...
  return {
    ...
    storeUrl: ensured.storeUrl || chromeWebStoreUrl(),
    applications: ensured.applications,
    ...(ensured.commit ? { commit: ensured.commit } : {}),
  };
}

function prepareWebBridge(_payload, deps = {}) {
  const installed = installWebBridgeFiles({
    fs: deps.fs,
    path: deps.path,
    home: deps.home,
    resourcesPath: deps.resourcesPath,
    sourceDir: deps.webBridgeSourceDir,
    platform: deps.platform,
    snapshot: deps.snapshot || {},
  });
  if (!installed || installed.status !== "ok") {
    return installed || { status: "error", message: "Could not prepare the browser extension." };
  }
  return {
    status: "ok",
    installDir: installed.installDir,
    version: installed.version,
    storeUrl: installed.storeUrl || chromeWebStoreUrl(),
    applications: installed.applications,
    commit: buildWebBridgeCommit(deps.snapshot || {}, installed.applications),
  };
}

function revealWebBridge(payload, deps = {}) {
  const pathApi = deps.path || require("path");
  const home = deps.home || require("os").homedir();
  const inspected = inspectWebBridgeInstall({
    fs: deps.fs,
    path: pathApi,
    home,
  });
  ...
  let opened = false;
  if (typeof deps.openExtensionsPage === "function") {
    try { opened = !!deps.openExtensionsPage(url, installDir); } catch { opened = false; }
  } else if (typeof deps.openExternal === "function") {
    try {
...
ensureWebBridge.lockKey = "agentIntegration";
prepareWebBridge.lockKey = "agentIntegration";
...
async function _wslCommand(payload, deps, { commandName, depName, action }) {
  ...
  try {
    ...
    if (result && result.ok) {
      ...
      // deploy-only: false = hooks installed but AgentHalo is unreachable from
