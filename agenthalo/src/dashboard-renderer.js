...
function badgeLabel(badge) {
  return t(badge === "running" ? "sessionBadgeRunning" : "sessionBadgeIdle");
}
...
function createCard(session, now) {
  ...
  button.textContent = focusTargetType === "codex-thread"
    ...
    : focusTargetType === "workbuddy-task"
    ? t("dashboardOpenApplicationSession")
    : focusTargetType === "web-chat"
    ? t("dashboardOpenWebSession")
  ...
  if (session.canFocus === true) actions.appendChild(button);
...
function render(options = {}) {
  ...
  const sessions = Array.isArray(snapshot.sessions)
    ? snapshot.sessions.filter(session => !session.hiddenFromSessionList && !session.headless)
    : [];
