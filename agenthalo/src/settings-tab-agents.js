...
(function initSettingsTabAgents(root) {
  ...
  function render(parent) {
    ...
    if (root.ClawdSettingsDoctorModal) {
      const health = document.createElement("div");
      health.className = "settings-connection-health";
  ...
  // A catalog of what AgentHalo supports, not a to-do list — collapsed by default
  ...
  function isWebBridgeCustomAgent(agent) {
    return !!(agent && agent.custom && (
      agent.processName === "claude-web"
      || agent.processName === "chatgpt-web"
      || agent.processName === "gemini-web"
      || agent.name === "Claude Web"
      || agent.name === "ChatGPT Web"
      || agent.name === "Gemini Web"
    ));
  }
  ...
  function categorizeAgentsForSections(agents) {
    ...
    for (const agent of agents) {
      ...
      if (agent.custom) {
        if (isWebBridgeCustomAgent(agent)) continue;
  ...
  function buildAgentMasterRow(agent) {
    ...
    return buildAgentSwitchRow({
      ...
      buildText: (text) => {
        ...
        text.appendChild(label);
        const descriptionKey = agent.id === "qwen-code" ? "appQwenCodeDescription"
          : agent.id === "qwenwork" ? "appQwenWorkDescription"
          : agent.id === "copilot-cli" ? "appCopilotDescription"
          : null;
        if (descriptionKey) {
        ...
        // TraeCode requires a manual enable step inside Trae that AgentHalo cannot
  ...
  function buildAgentDetailRows(agent) {
    ...
    if (!agent.custom) {
      rows.push(buildAgentConfigDirRow(agent));
    }
    if (agent.custom) {
        ...
        if (window.settingsAPI && typeof window.settingsAPI.command === "function") {
          ...
          if (result && result.status === "ok") {
            ...
            if (result.wslConnectivity === false) {
              // Hooks installed, but the distro cannot reach AgentHalo (NAT
  ...
  function authorizedDirFor(agentId) {
    const dirs = (runtime.authorizedDirs && typeof runtime.authorizedDirs === "object")
      ? runtime.authorizedDirs
      : {};
    return dirs[agentId] || null;
  }

  function refreshAuthorizedDirs() {
    if (!window.settingsAPI || typeof window.settingsAPI.command !== "function") {
      return Promise.resolve();
    }
  ...
  function buildAgentConfigDirRow(agent) {
    const row = document.createElement("div");
    row.className = "row row-sub agent-config-dir-row";
    const text = document.createElement("div");
    text.className = "row-text";
    const label = document.createElement("span");
    label.className = "row-label";
    ...
    const desc = document.createElement("span");
    desc.className = "row-desc";
    desc.textContent = t("agentConfigDirHint");
    text.appendChild(label);
    text.appendChild(desc);
    row.appendChild(text);
    const button = document.createElement("button");
    button.type = "button";
    button.className = "soft-btn agent-config-dir-authorize";
    button.textContent = t(authorized ? "agentConfigDirChange" : "agentConfigDirAuthorize");
    button.addEventListener("click", async (ev) => {
      ev.stopPropagation();
      if (button.disabled) return;
      button.disabled = true;
      try {
        const result = await window.settingsAPI.command("authorizeAgentConfigDir", {
          agentId: agent.id,
          force: true,
        });
        if (!result || result.status !== "ok") {
  ...
  function buildAgentIntegrationActionButton(agent) {
    ...
    button.addEventListener("click", (ev) => {
      ...
      if (installed && agent.id !== "claude-code" && typeof window.confirm === "function" && !window.confirm(t("agentIntegrationUninstallConfirm"))) {
        return;
      }
      button.disabled = true;
      button.classList.add("pending");
      button.textContent = t("agentIntegrationWorking");
      const request = installed && agent.id === "claude-code"
        ? runDisconnectClaudeHooks()
        : window.settingsAPI.command(command, { agentId: agent.id });
      request.then((result) => {
