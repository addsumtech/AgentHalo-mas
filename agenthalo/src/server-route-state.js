...
function sendWebBridgeDiscoveryResponse(res, options = {}) {
  const { buildWebBridgeDiscovery } = require("./web-bridge-install");
  const body = JSON.stringify(buildWebBridgeDiscovery(options.applications || [], {
    appId: CLAWD_SERVER_ID,
  }));
  res.writeHead(200, {
    "Content-Type": "application/json",
    [CLAWD_SERVER_HEADER]: CLAWD_SERVER_ID,
...
function handleStatePost(req, res, options) {
  ...
  // See the matching note on the permission route: concatenating Buffers into a
  // string decodes each chunk separately and mangles any multi-byte character
  // that straddles a chunk boundary. cwd and assistant_last_output both carry
  // CJK routinely. Byte counting stays on the raw chunks.
  const bodyChunks = [];
  let bodySize = 0;
  let tooLarge = false;
  req.on("data", (chunk) => {
  ...
  req.on("end", () => {
    ...
    try {
      ...
      if (agentId === "deepseek-harness") {
        const sequenceResult = dshStateSequenceFence
          ...
          ? dshStateSequenceFence.accept({
              ...
              // AgentHalo storage concern applied by resolveSessionIdentity.
      ...
      // deployed AgentHalo hooks to. The store shape-sanitizes the label.
      ...
      if (metadataOnly) {
        ...
        if (typeof ctx.updateSessionMetadata === "function") {
          ...
          // maps to no AgentHalo state). Not gated on the Claude telemetry flag —
      ...
      // A bare lookup also matches inherited keys, so "constructor", "toString"
      // and "__proto__" passed as valid states and reached the session map.
      if (Object.prototype.hasOwnProperty.call(ctx.STATE_SVGS, state) && ctx.STATE_SVGS[state]) {
        const sid = session_id || "default";
        ...
        if (isCodexOfficialSessionOpenOnly(data, !!existingSession)) {
          recordRequestHookEvent.droppedUnsupported();
          res.writeHead(204, { [CLAWD_SERVER_HEADER]: CLAWD_SERVER_ID });
          res.end();
          return;
        }
        // Codex Desktop spawns child threads inside a parent's turn. They fire
        // the full official lifecycle but send no parent or role marker, so the
        // subagent classifier cannot see them; judged at the end of their turn,
        // a thread nothing on disk knows about is one of those. Deciding at the
        // turn's end rather than on arrival gives a real conversation the whole
        // turn for its rollout file to appear.
        //
        // Classify BEFORE the empty-turn rewrite below: that rewrite also
        // consumes a Stop, and testing `event === "Stop"` afterwards made the
        // two mitigations mutually exclusive.
        let codexChildThread = false;
        if (agentId === "codex" && event === "Stop" && data.hook_source === "codex-official") {
          // Codex's own `threads` table is authoritative: it marks subagents and
          // omits the ChatGPT app's code-mode tool runs entirely. Only when it
          // cannot be read do we fall back to the old file-based heuristic.
          const verdict = classifyCodexThread(
            typeof ctx.lookupCodexThread === "function"
              ? ctx.lookupCodexThread(sessionIdentity.rawSessionId)
              : null
          );
          if (verdict === "unknown") {
            codexChildThread = typeof ctx.hasCodexJsonlSession === "function"
              && isCodexChildThread({
                jsonlSeen: ctx.hasCodexJsonlSession(sid),
                cwd: cwd || (existingSession && existingSession.cwd) || "",
                sessionTitle: sessionTitle || (existingSession && existingSession.sessionTitle) || "",
              });
          } else {
            codexChildThread = verdict === "hidden";
          }
          if (typeof ctx.debugLog === "function") {
            ctx.debugLog(
              `codex-thread-class sid=${String(sid).replace(/[\r\n]/g, "_")}`
              + ` verdict=${verdict} headless=${codexChildThread ? 1 : 0}`
            );
          }
        }
        // Retire rather than park: an empty turn on a session nothing ever
        // identified leaves a card that no exit probe can ever clear.
        if (isCodexOfficialEmptyTurnEnd(data, state, existingSession, codexHookState.headless === true)) {
          event = "SessionEnd";
          state = "sleeping";
        }
        // A Cursor subagent reports its own conversation, which would otherwise
        ...
        // their hook with "{}"/no-decision when AgentHalo has no real user
        ...
        if (svg) {
          ...
          ctx.updateSession(sid, state, event, {
            ...
            host,
            wslDistro,
            headless: headless || codexHookState.headless === true || cursorSubagent || codexChildThread,
            platform,
            model,
            provider,
            codexOriginator,
...
module.exports = {
  ...
  sendWebBridgeDiscoveryResponse,
  handleStatePost,
};
