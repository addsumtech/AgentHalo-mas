...
const { createCodexJsonlSessions } = require("./codex-jsonl-sessions");
...
function createAgentRuntimeMain(options = {}) {
  ...
  const workBuddyMonitor = (options.createWorkBuddySessionMonitor
    || require("./workbuddy-session-monitor").createWorkBuddySessionMonitor)({
    getSessions: () => { const state = getStateRuntime(); return state && state.sessions; },
    isEnabled: () => isAgentEnabled("workbuddy"),
    onClosed: (sid, session) => updateSession(sid, "idle", "SessionEnd", {
      agentId: "workbuddy", profileId: "local", rawSessionId: session.rawSessionId,
  ...
  const codexJsonlSessions = createCodexJsonlSessions({ now });
  ...
  // Only a thread with a rollout file reaches us through JSONL, so this is what
  // separates a conversation the user opened from a child Codex Desktop spawned
  // inside one. See codex-jsonl-sessions.js.
  function markCodexJsonlSession(sessionId) {
    codexJsonlSessions.mark(sessionId);
  }

  function hasCodexJsonlSession(sessionId) {
    return codexJsonlSessions.has(sessionId);
  }
  ...
  function updateSessionFromServer(sessionId, state, event, opts = {}) {
    if (opts.agentId === "workbuddy" && !opts.host) {
      ...
      const existing = runtime && runtime.sessions && runtime.sessions.get(sessionId);
      if (event === "SessionStart" && existing && existing.agentId === "workbuddy") {
        opts = { ...opts, preserveState: true };
      }
    }
    if (
      opts.agentId === "codex" && opts.profileId === "local" && !opts.host
      && codexMonitor && typeof codexMonitor.isSessionArchived === "function"
      && codexMonitor.isSessionArchived(opts.rawSessionId || sessionId)
    ) return false;
  ...
  function startMonitorForAgent(agentId) {
    if (agentId === "workbuddy") workBuddyMonitor.start();
    if (agentId === "codex" && codexMonitor) codexMonitor.start();
  }

  function stopMonitorForAgent(agentId) {
    if (agentId === "workbuddy") workBuddyMonitor.stop();
  ...
  function closeCodexSession(sid, reason) {
    const identity = resolveSessionIdentity(sid, "local");
    const stateRuntime = getStateRuntime();
    const session = stateRuntime && stateRuntime.sessions
      && stateRuntime.sessions.get(identity.sessionId);
    clearCodexNotifyBubbles(identity.sessionId, reason);
    clearCodexUserInputBubbles(identity.sessionId, null, reason);
    if (!session || session.agentId !== "codex" || session.host) return;
    updateSession(identity.sessionId, "idle", "SessionEnd", {
      agentId: "codex",
  ...
  function startCodexLogMonitor() {
    ...
    try {
      ...
      codexMonitor = new CodexLogMonitor(codexAgent, (sid, state, event, extra) => {
        ...
        markCodexJsonlSession(sessionId);
        ...
        onSessionArchived: (sid) => closeCodexSession(sid, "codex-session-archived"),
        onSessionClosed: (sid, reason) => closeCodexSession(sid, reason),
      ...
      logWarn("AgentHalo: Codex log monitor not started:", err && err.message);
    }
    return codexMonitor;
  }

  function cleanup() {
    workBuddyMonitor.stop();
