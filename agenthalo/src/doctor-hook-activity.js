...
function evaluateConnectionTest(input = {}) {
  ...
  if (codexActivity && codexHookHealth && codexHookHealth.signature === "needs-review") {
    return {
      ...
      level: "warning",
      detail: "Codex activity was detected, but AgentHalo cannot evaluate the HTTP path because its Codex hooks still need review. Run /hooks in Codex CLI, review the AgentHalo hooks, then run this test again. This result does not rule out a separate firewall or proxy issue.",
    };
  }
  ...
  if (fileActivity.length) {
    ...
    return {
      status: "http-blocked",
      level: "warning",
      detail: `File activity changed for ${agents.join(", ")}, but no HTTP hook event reached AgentHalo. Firewall, EDR, or proxy interception is likely.`,
    };
  }
