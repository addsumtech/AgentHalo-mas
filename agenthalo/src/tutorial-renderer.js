...
(function () {
    ...
    try {
      ...
      if (failed > 0) {
        ...
        setAgentNotice(kind, "ok", "tutorialAgentsCleanupDone",
          "Done. The selected AgentHalo connections were disconnected.", { count: ok });
      } else {
        setAgentNotice(kind, "ok", "tutorialAgentsInstallDone",
          "Done. AgentHalo is listening to the selected tools.", { count: ok });
      }
    } finally {
  ...
  function renderWelcome() {
    ...
    wrap.appendChild(el("h2", { class: "step-title" }, i18n("tutorialWelcomeTitle", "Welcome to AgentHalo")));
    wrap.appendChild(el("p", { class: "step-sub" }, i18n("tutorialWelcomeBody",
      "AgentHalo follows the AI tools you choose, then reacts on your desktop when they work, wait for approval, or finish.")));
  ...
  function renderAgents() {
    ...
    const wrap = el("div", {});
    wrap.appendChild(el("h2", { class: "step-title" }, i18n("tutorialAgentsTitle", "Let AgentHalo follow your AI tools")));
    wrap.appendChild(el("p", { class: "step-sub" }, i18n("tutorialAgentsSub",
      "This only changes AgentHalo's connection. It won't install or remove the tools themselves.")));
    ...
    if (ag.cleanup.length) {
      wrap.appendChild(actionPanel("cleanup", ag.cleanup,
        ...
        "tutorialAgentsCleanupDesc", "These AgentHalo connections still exist, but the tools were not found. Disconnect the ones you no longer use.",
      ...
      const panel = el("section", { class: "ag-panel active" },
        el("div", { class: "ag-panel-head" },
          el("div", { class: "ag-panel-copy" },
            ...
            el("p", { class: "ag-panel-desc" }, i18n("tutorialAgentsActiveDesc", "AgentHalo is set up to listen to these tools.")))));
  ...
  function fixedShortcuts() {
    return [
      ...
      { key: "shortcutLabelPetReveal", fallback: "Bring AgentHalo forward", keys: i18n("tutorialShortcutClickPet", "Click pet") },
      {
  ...
  function renderShortcuts() {
    ...
    wrap.appendChild(renderShortcutSection(
      ...
      "tutorialShortcutsEditableDesc", "Pick the shortcuts you want. AgentHalo checks conflicts when you save.",
  ...
  function renderFeatures() {
    ...
    wrap.appendChild(el("p", { class: "step-sub" }, i18n("tutorialFeaturesSub",
      "Nothing here is required for setup. These are handy once AgentHalo is running.")));
    const platformStarter = isMac()
      ...
      : featureCard("tutorialFeatureDrag", "Drop a project folder",
        "tutorialFeatureDragDesc", "Drop a folder on AgentHalo to open a terminal in that directory.",
  ...
  function renderDone() {
    ...
    wrap.appendChild(el("p", { class: "step-sub" }, i18n("tutorialDoneBody",
      "Start a connected AI tool and AgentHalo will react on your desktop. You can reopen this guide from Settings → About.")));
    wrap.appendChild(el("div", { style: "margin-top:14px" },
      inlineLink(i18n("tutorialDoneOpenSettings", "Open Settings"),
        () => api.openSettingsTab && api.openSettingsTab("about"))));
    return wrap;
  }
  ...
  function render() {
    ...
    document.title = i18n("tutorialWindowTitle", "Welcome to AgentHalo");
    renderSteps();
    const body = document.getElementById("body");
