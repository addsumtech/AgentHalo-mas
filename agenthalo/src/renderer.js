...
function noteAccessoryDiagnostic(slotName, file, reason) {
  ...
  try { console.warn(`AgentHalo: ${slotName} accessory fallback for ${file || "unknown"}: ${reason}`); } catch {}
}
...
/**
 * Determine if a state should attach AgentHalo-controlled eye tracking.
 */
...
function playReaction(requestOrFile, durationMs) {
  ...
  // A second reaction arriving mid-play used to leave the first timer armed,
  // and it then fired endReaction() into the running one and cut it short.
...
function warnDirectionalDragBridgeOnce(reason) {
  ...
  console.warn(`AgentHalo: directional drag bridge unavailable (${reason}); keeping the fallback direction.`);
}
...
function warnCodexPetVisualBridgeOnce(reason) {
  ...
  console.warn(`AgentHalo: Codex Pet visual bridge unavailable (${reason}); using a normal media swap.`);
}
...
function forceImageChannelReload(file, state, allowImageFallback = true, options = {}) {
  ...
  console.warn("AgentHalo: animation stayed invisible; reloading through the image channel:", file);
...
function renderStateFile(requestOrState, legacySvg) {
  ...
  if (requestOrState && typeof requestOrState === "object" && !visualRequest) {
    try { console.warn("AgentHalo: ignored malformed visual request"); } catch {}
    return;
  }
...
function reportSoundPlaybackError(phase, err) {
  ...
  try { console.warn(`AgentHalo sound ${phase} failed:`, message); } catch {}
}
...
const soundPlayback = window.AgentHaloSoundPlayback.createSoundPlayback(
  () => window.electronAPI.isSystemAudioMuted()
);

window.electronAPI.onPlaySound(async (payload) => {
  ...
  if (!url) return;
  try {
    if (await window.electronAPI.isSystemAudioMuted()) return;
  } catch { return; }
  ...
  warmAudioOutput(url).then(() => {
    audio.volume = volume;
    soundPlayback.play(audio).catch((err) => reportSoundPlaybackError("play", err));
  });
});
