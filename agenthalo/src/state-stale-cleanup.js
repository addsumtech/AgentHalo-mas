...
function getStaleSessionDecision(session, options = {}) {
  ...
  // A closed terminal without a separate agent PID is a positive exit signal
  // even when the last turn has already finished. Do not retain it for 10 min.
  if (!isWorkingLikeState(session.state) && session.pidReachable && !session.agentPid && session.sourcePid && !isProcessAlive(session.sourcePid)) {
