...
function checkCodexHookTrustText(configText, settings, hooksPath, options = {}) {
  ...
  if (missing.length) {
    ...
    return {
      key: "codex_hook_trust",
      value: "needs-review",
      detail: `${missing.length}/${positions.length} AgentHalo Codex hook(s) need Codex /hooks review: ${missingEvents}`,
  ...
  return {
    key: "codex_hook_trust",
    value: "trusted",
    detail: `${positions.length}/${positions.length} AgentHalo Codex hook(s) trusted by Codex`,
