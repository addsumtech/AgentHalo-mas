...
const GEMINI_HOOKS_DISABLED_DETAIL = "Gemini hooks are disabled in settings.json; AgentHalo preserves this user setting and will not receive hook events";
const ANTIGRAVITY_HOOKS_DISABLED_DETAIL = "Antigravity AgentHalo hooks are disabled in hooks.json; AgentHalo preserves this user setting and will not receive hook events";
const QWEN_HOOKS_DISABLED_DETAIL = "Qwen Code hooks are disabled in settings.json; AgentHalo preserves this user setting and will not receive hook events";
...
function withAgentBubbleNote(detail, prefs, agentId) {
  ...
  // surface a AgentHalo bubble in the first place, so annotating them as
...
function withClaudeHookGuardNotice(detail, descriptor, options) {
  ...
  if (
    ...
    return {
      ...
      level: "warning",
      detail: "The current AgentHalo installation's Claude hook script is missing. Reinstall or re-extract AgentHalo to restore automatic hook repair.",
  ...
  if (
    ...
    return {
      ...
      detail: unresolvedNode
        ? "AgentHalo preserved an env-indirected Claude hook because settings.env does not provide a usable absolute Node path. Check CLAWD_NODE_BIN, then use Fix."
        : "AgentHalo found an env-indirected Claude hook but settings.env does not prove that it belongs to AgentHalo. Review CLAWD_HOOK_PATH before changing it.",
      claudeHookRuntimeStatus: {
        status: runtimeHealth.status,
        degradedReason: runtimeHealth.degradedReason,
        at: runtimeHealth.at || null,
  ...
  if (guard && guard.type === "suspicious-shrink") {
    return {
      ...detail,
      detail: "AgentHalo paused automatic Claude hook repair after settings.json shrank during an external rewrite. Use Fix or restart AgentHalo to reinstall AgentHalo hooks.",
      claudeHookGuard: {
        type: guard.type,
        at: guard.at || null,
  ...
  if (runtimeHealth && runtimeHealth.status === "guarded") {
    return {
      ...detail,
      detail: "AgentHalo paused automatic Claude hook repair because settings.json shrank suspiciously. Use Fix or restart AgentHalo to reinstall AgentHalo hooks.",
      claudeHookRuntimeStatus: {
        status: runtimeHealth.status,
  ...
  if (runtimeHealth && runtimeHealth.status === "manual-fix-required") {
    return {
      ...detail,
      detail: "AgentHalo's automatic Claude hook repair failed 3 times in a row and stopped retrying. Use Fix to try once more, or check the hook script manually.",
      claudeHookRuntimeStatus: {
        status: runtimeHealth.status,
        issueSignature: runtimeHealth.issueSignature || null,
...
// and picks a run mode — the IDE holds that switch in private storage AgentHalo
...
function withAgentFixAction(detail, descriptor) {
  if (
    ...
    // strips and rewrites every AgentHalo block in the current canonical form.
  ...
  if (
    ...
    // A Fix would re-register AgentHalo's blocking hook next to the user's own
  ...
  if (
    ...
    // disabled-*: user set disableAllHooks; AgentHalo must not override.
    ...
    // without a button so the user wires AgentHalo in manually if they want it.
    return detail;
  }
...
function validateCopilotHookEvents(descriptor, settings, settingsJson, options) {
  ...
  if (settings && settings.disableAllHooks === true) {
    return makeDetail(descriptor, "not-connected", {
      level: "warning",
      detail: `${descriptor.configPath} has disableAllHooks=true; AgentHalo hooks will not run`,
  ...
  if (settingsJson && settingsJson.disableAllHooks === true) {
    return makeDetail(descriptor, "not-connected", {
      level: "warning",
      detail: `${descriptor.settingsPath || "settings.json"} has disableAllHooks=true; AgentHalo hooks will not run`,
  ...
  //   - in-file: another (non-AgentHalo) entry in hooks.json's permissionRequest
...
function getGeminiHooksSupplementary(settings, descriptor) {
  ...
  if (!hooksConfig || typeof hooksConfig !== "object") {
    return {
      key: "gemini_hooks",
      value: "enabled",
      detail: "hooksConfig allows AgentHalo Gemini hooks",
    };
  }
...
function applyDisabledHookGroup(detail, descriptor, settings) {
  ...
  return {
    ...
    status: "not-connected",
    level: "warning",
    detail: `${descriptor.agentName} hooks are disabled in settings.json; AgentHalo preserves this user setting and will not receive hook events`,
    supplementary,
  };
}
...
function getQwenHooksSupplementary(settings) {
  ...
  return {
    key: "qwen_hooks",
    value: "enabled",
    detail: "settings.json allows AgentHalo Qwen hooks",
  };
}
...
function getZcodeHooksSupplementary(settings, descriptor) {
  ...
  if (disabledEvents.length > 0) {
    ...
    // opt-out AgentHalo's own hooks (the would-be conflicting side) are inactive,
    // so the disabled-events report is the actionable one.
    return {
      key: "zcode_hooks",
      value: "disabled-events",
      detail: `AgentHalo hooks have enabled=false for: ${disabledEvents.join(", ")}`,
      disabledEvents,
    };
  }
  if (invalidWrapperEvents.length > 0) {
    ...
    return {
      ...
      detail: `AgentHalo hook wrapper has unsupported fields for: ${uniqueEvents.join(", ")}`,
  ...
  // last-wins decisions, so a non-AgentHalo hook owning PermissionRequest means
  // AgentHalo's blocking hook must stay unregistered — and a Fix that silently
  ...
  for (const entry of permissionEntries) {
    ...
    for (const hook of entry.hooks) {
      ...
      // Mirror hooks/zcode-install.js exactly: any nested non-AgentHalo object is
  ...
  if (foreignPermissionHook) {
    return {
      ...
      detail: managedPermissionHook
        ? "a foreign PermissionRequest hook coexists with AgentHalo's; ZCode executes same-event hooks with last-wins decisions — keep exactly one owner for PermissionRequest"
        : "a foreign PermissionRequest hook owns the event; AgentHalo's blocking permission hook is intentionally not registered (last-wins would override the existing hook's deny)",
    };
  }
  ...
  return {
    key: "zcode_hooks",
    value: "enabled",
    detail: "config.json allows AgentHalo ZCode hooks",
  };
}
...
function applyZcodeSupplementary(detail, descriptor, settings) {
  ...
  if (supplementary.value === "disabled-global") {
    return {
      ...
      status: "not-connected",
      level: "warning",
      detail: "ZCode config-file hooks are disabled globally; AgentHalo preserves hooks.enabled=false and will not receive hook events",
      supplementary,
    };
  }
  if (supplementary.value === "disabled-events") {
    return {
      ...
      status: "not-connected",
      level: "warning",
      detail: `AgentHalo ZCode hooks are disabled for ${supplementary.disabledEvents.join(", ")}; AgentHalo preserves each enabled=false setting`,
      supplementary,
    };
  }
  if (supplementary.value === "needs-enable") {
    return {
      ...
      status: "not-connected",
      level: "warning",
      detail: "ZCode config-file hooks require hooks.enabled=true before AgentHalo can receive events",
      supplementary,
    };
  }
  if (supplementary.value === "invalid-wrapper") {
    return {
      ...
      status: "not-connected",
      level: "warning",
      detail: `ZCode rejected unsupported fields on AgentHalo hook wrappers for ${supplementary.invalidWrapperEvents.join(", ")}`,
      supplementary,
    };
  }
...
function getAntigravityHooksSupplementary(settings) {
  ...
  return {
    key: "antigravity_hooks",
    value: "enabled",
    detail: "hooks.json allows AgentHalo Antigravity hooks",
  };
}
...
function validateCodewhaleHookEvents(descriptor, text, options) {
  ...
  if (codewhaleHooksExplicitlyDisabled(text)) {
    return makeDetail(descriptor, "not-connected", {
      level: "warning",
      detail: "CodeWhale hooks are disabled in config.toml; AgentHalo will not receive hook events",
      supplementary: {
        key: "codewhale_hooks",
        value: "disabled",
        detail: "[hooks].enabled is false",
...
function checkKiroDirMode(descriptor, options) {
  ...
  return makeDetail(descriptor, "not-connected", {
    ...
    configFileExists: true,
    configPath: agentsDir,
    detail: "No Kiro agent config contains a valid AgentHalo hook",
    kiroScan: scan,
  });
}
...
function checkOpenClawPluginMode(descriptor, options) {
  ...
  try {
    ...
    return makeDetail(descriptor, "needs-review", {
      ...
      configFileExists: true,
      configPath: descriptor.configPath,
      detail: `OpenClaw config is not strict JSON; AgentHalo startup sync will skip direct edits (${err && err.message ? err.message : "parse failed"})`,
    });
  }
  ...
  if (hasIncludeDirective(settings)) {
    return makeDetail(descriptor, "needs-review", {
      ...
      configFileExists: true,
      configPath: descriptor.configPath,
      detail: "OpenClaw config uses include directives; AgentHalo startup sync will not edit it directly",
    });
  }
  ...
  if (pluginConfig && pluginConfig.enabled === false) {
    return makeDetail(descriptor, "not-connected", {
      ...
      configFileExists: true,
      configPath: descriptor.configPath,
      detail: "OpenClaw AgentHalo plugin is registered but disabled",
      openclawEntry: entry,
    });
  }
...
function checkPiExtensionMode(descriptor, options) {
  ...
  if (!isPiManagedMarker(marker)) {
    return makeDetail(descriptor, "needs-review", {
      ...
      extensionDir,
      markerPath,
      detail: `${extensionDir} exists but is not AgentHalo-managed`,
    });
  }
    ...
    if (health.status === "healthy") {
      ...
      detail = makeDetail(descriptor, "needs-review", {
        ...
        detail: health.status === "version-unsupported" || health.status === "host-version-unsupported"
          ? `DeepSeek Harness ${health.detectedDshVersion || "or its bridge marker"} is outside ${health.supportedDshRange || "the supported range"}; AgentHalo will not activate it automatically`
          : (health.status === "generation-integrity-failed"
            ? "The managed DeepSeek Harness bridge bytes no longer match their ownership marker; inspect them manually"
            : (health.status === "source-unavailable"
              ? "AgentHalo's packaged DeepSeek Harness bridge source is unavailable; repair the AgentHalo installation"
              : (health.status === "profile-corrupt"
                ? "The DeepSeek Harness web profile manifest is unreadable; AgentHalo will not rewrite it automatically"
                : "A foreign or conflicting DeepSeek Harness plugin uses the AgentHalo bridge package name; AgentHalo will not replace it"))),
        configPath: health.profileDir,
        bridgeHealth: health.status,
      });
    } else {
...
// whenever the legacy directory carries AgentHalo hooks, whichever target the
...
function withKimiLegacyPermissionModeSupplement(detail, descriptor, options) {
  ...
  // No AgentHalo hooks on legacy at all: connection status is the primary
  ...
  return {
    ...
    status: "needs-review",
    level: "warning",
    detail: `${legacyTarget.configPath}: ${issues.join("; ")} — Fix rewrites AgentHalo's Kimi hooks in the current format`,
