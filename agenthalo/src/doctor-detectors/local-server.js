...
function checkLocalServer(serverApi) {
  ...
  if (!status || !status.listening) {
    return {
      ...
      textHint: "Restart AgentHalo. If the issue persists, check ~/.clawd/ permissions.",
      runtime: status,
  ...
  // dead/foreign owner, reads as "AgentHalo is offline" to every hook — so terminal
  ...
  if (!status.runtimeFileExists || !status.runtimeMatches || identityBroken) {
    ...
    return {
      ...
      textHint: "Restart AgentHalo to regenerate the runtime file.",
      runtime: status,
