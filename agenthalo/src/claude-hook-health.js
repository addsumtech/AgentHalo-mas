...
// Validates every AgentHalo-owned command found for one event. Every command is
...
function inspectEventCommands(commands, event, marker, expectedScriptPath, validateOptions, issues, issueCodes) {
  ...
  for (const result of results) {
    ...
    if (result.issue === "nodeBin-invalid") {
      ...
      // AgentHalo does not own; surface it for Doctor instead.
      pushIssue(issues, {
 ...
 * as AgentHalo's to rewrite is worse than leaving it alone), so there is no
...
/**
 * Whether a report contains a AgentHalo-owned command this module could not
...
const DEGRADED_DIAGNOSTICS = Object.freeze({
  "command-unparseable": Object.freeze({
    ...
    message: "a AgentHalo-owned hook command could not be parsed; see Doctor for details",
  }),
  "env-hook-node-unresolved": Object.freeze({
    reason: "env-hook-node-unresolved",
    message: "an env-indirected AgentHalo hook was preserved because its absolute Node path could not be verified",
  }),
  "env-indirection-unverified": Object.freeze({
    reason: "env-indirection-unverified",
    message: "an env-indirected hook command could not be proven AgentHalo-owned from settings.env",
  }),
});
 ...
 * or an unparseable AgentHalo-owned command.
 */
