...
function shouldAutoClearDetachedSession(session, badge, options = {}) {
  ...
  // Hiding a card is a one-way door for the user, so never rest it on a pid
  // issued by another machine's namespace (WSL, SSH).
...
function sessionDisplayFolder(id, sessionLike) {
  ...
  // Windows agent reported to a macOS/Linux AgentHalo). Select the path dialect
...
// True when the label is only the workspace folder, because nothing named this
// session. Consumers mark it so a folder cannot pass for a task title.
function sessionTitleIsFolder(id, sessionLike, sessionAliases = {}, options = {}) {
  const alias = getSessionAliasEntry(id, sessionLike, sessionAliases);
  if (alias && typeof alias.title === "string" && alias.title) return false;
  if (getEffectiveSessionTitle(id, sessionLike, options)) return false;
  return !!sessionDisplayFolder(id, sessionLike);
}
...
function isBackgroundChildSession(session) {
  if (!session) return false;
  if (session.headless === true) return true;
  if (session.agentId !== "codex") return false;
  const source = String(session.codexSource || "").trim();
  const isExec = source === "exec" || (!source && session.codexOriginator === "codex_exec");
  if (!isExec) return false;
  // Claude Code's scratchpad includes the owning Claude session UUID. This
  // provenance survives detached child processes being reparented to launchd.
  // Ordinary Codex CLI/Desktop sessions and generic folders named A/B do not
  // match. This is a presentation rule, never a permission/headless decision.
  return /^(?:\/private)?\/tmp\/claude-\d+\/[^/]+\/[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}\/scratchpad(?:\/|$)/i
    .test(String(session.cwd || ""));
}
...
function buildSessionSnapshotEntry(id, session, sessionAliases = {}, options = {}) {
  ...
  const hiddenFromSessionList = isBackgroundChildSession(session);
  const hiddenFromHud = hiddenFromSessionList || shouldAutoClearDetachedSession(session, badge, options)
  ...
  return {
    ...
    titleIsFolder: sessionTitleIsFolder(id, session, sessionAliases, options),
...
function buildSessionSnapshot(sessions, options = {}) {
  ...
  const visibleEntries = entries.filter(entry => !entry.hiddenFromSessionList);
  const dashboardEntries = visibleEntries.slice().sort(sessionUpdatedAtComparator);
  const menuEntries = visibleEntries.slice().sort((a, b) => sessionMenuComparator(a, b, options.statePriority));
...
function sessionSnapshotSignature(snapshot) {
  return JSON.stringify({
    ...
    sessions: snapshot.sessions.map((entry) => ({
      ...
      hiddenFromSessionList: !!entry.hiddenFromSessionList,
      host: entry.host,
