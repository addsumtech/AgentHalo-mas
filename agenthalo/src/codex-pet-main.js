...
const { localizeField } = require("./localized-field");
...
function extractClawdProtocolUrls(argv) {
  ...
  return argv.filter((arg) => typeof arg === "string" && IMPORT_PROTOCOL_SCHEMES.some((scheme) => arg.toLowerCase().startsWith(`${scheme}:`)));
}
...
function createCodexPetMain(options = {}) {
  ...
  function getActiveThemeId() {
    ...
    return activeTheme ? activeTheme._id : (settingsController.get("theme") || "halo");
  }
  ...
  function rebuildMenusBestEffort(optionsForRebuild = {}) {
    ...
    try {
      ...
      console.warn("AgentHalo: rebuildAllMenus after Codex Pet refresh failed:", err && err.message);
    }
  }
  ...
  function syncThemes(activeThemeId) {
    try {
      ...
      lastSyncSummary = summary;
      console.warn("AgentHalo: failed to sync Codex Pet themes:", err && err.message);
      return summary;
    }
  }
  ...
  async function refreshFromSettings() {
    ...
    if (summaryHasActiveCodexPetOrphan(summary, activeId)) {
      const result = await settingsController.applyCommand("setThemeSelection", { themeId: "halo" });
      if (!result || result.status !== "ok") {
        return {
          status: "error",
          message: (result && result.message) || "failed to switch active orphan Codex Pet theme back to halo",
          summary,
        };
      }
      switchedToFallback = true;
      const cleanup = syncThemes("halo");
  ...
  function registerProtocolClient() {
    try {
      ...
      console.warn("AgentHalo: failed to register agenthalo:// protocol:", err && err.message);
    }
    return false;
  }
  ...
  function getImportDialogStrings() {
    const all = {
      en: {
        ...
        confirmDetail: (url) => `AgentHalo will download, validate, and install this pet package before switching to it.\n\n${url}`,
      ...
      zh: {
        ...
        confirmDetail: (url) => `AgentHalo 会先下载、校验并安装这个宠物包，然后切换到它。\n\n${url}`,
      ...
      "zh-TW": {
        ...
        confirmDetail: (url) => `AgentHalo 會先下載、驗證並安裝這個寵物套件，然後切換到它。\n\n${url}`,
      ...
      ko: {
        ...
        confirmDetail: (url) => `AgentHalo가 이 펫 패키지를 다운로드, 검증, 설치한 뒤 전환합니다.\n\n${url}`,
      ...
      ja: {
        ...
        confirmDetail: (url) => `AgentHalo はこのペットパッケージをダウンロード、検証、インストールしてから切り替えます。\n\n${url}`,
      ...
      "pt-BR": {
        ...
        confirmDetail: (url) => `O AgentHalo vai baixar, validar e instalar este pacote de pet antes de ativá-lo.\n\n${url}`,
      ...
      es: {
        ...
        confirmDetail: (url) => `AgentHalo descargará, validará e instalará este paquete de mascota antes de cambiar a él.\n\n${url}`,
  ...
  async function confirmReplaceExistingPackage(payload) {
    ...
    try {
      ...
      console.warn("AgentHalo: Codex Pet replace confirmation failed:", err && err.message);
      return false;
    }
  }
  ...
  function getRemovalDialogStrings() {
    const all = {
      en: {
        ...
        detail: "AgentHalo will remove the source package from your Codex pets folder and clean up the generated theme. This cannot be undone.",
      },
      zh: {
        uninstall: "卸载",
        cancel: "取消",
        message: (name) => `卸载导入宠物 "${name}"？`,
        detail: "AgentHalo 会从 Codex pets 文件夹删除源包，并清理生成的主题。此操作不可撤销。",
      },
      "zh-TW": {
        uninstall: "解除安裝",
        cancel: "取消",
        message: (name) => `解除安裝匯入的寵物「${name}」？`,
        detail: "AgentHalo 會從 Codex pets 資料夾移除來源套件，並清理產生的主題。此操作無法復原。",
      },
      ko: {
        uninstall: "제거",
        cancel: "취소",
        message: (name) => `가져온 펫 "${name}"을(를) 제거할까요?`,
        detail: "AgentHalo가 Codex pets 폴더의 원본 패키지를 제거하고 생성된 테마를 정리합니다. 이 작업은 되돌릴 수 없습니다.",
      },
      ja: {
        uninstall: "アンインストール",
        cancel: "キャンセル",
        message: (name) => `インポート済みペット "${name}" をアンインストールしますか？`,
        detail: "AgentHalo は Codex pets フォルダから元パッケージを削除し、生成されたテーマをクリーンアップします。この操作は元に戻せません。",
      },
      "pt-BR": {
        uninstall: "Desinstalar",
        cancel: "Cancelar",
        message: (name) => `Desinstalar o pet importado "${name}"?`,
        detail: "O AgentHalo vai remover o pacote de origem da sua pasta de Codex pets e limpar o tema gerado. Isso não pode ser desfeito.",
      },
      es: {
        uninstall: "Desinstalar",
        cancel: "Cancelar",
        message: (name) => `¿Desinstalar la mascota importada "${name}"?`,
        detail: "AgentHalo eliminará el paquete de origen de tu carpeta de Codex pets y limpiará el tema generado. Esta acción no se puede deshacer.",
      },
    };
    return all[getLang()] || all.en;
  }
  ...
  async function confirmRemoveImportedPackage(displayName) {
    ...
    try {
      ...
      console.warn("AgentHalo: Codex Pet removal confirmation failed:", err && err.message);
      return false;
    }
  }
  ...
  async function materializeAndActivateImportedPet(imported) {
    ...
    if (!generated || !generated.themeId) {
      throw new Error("imported package did not materialize into a AgentHalo theme");
    }
  ...
  async function handleImportProtocolUrl(rawUrl) {
    ...
    try {
      ...
      console.warn("AgentHalo: Codex Pet import confirmation failed:", err && err.message);
      return;
    }
    ...
    try {
      ...
      console.warn("AgentHalo: Codex Pet import failed:", err && err.message);
      await showImportError(err && err.message);
    }
  }
  ...
  async function openCodexPetsDir() {
    try {
      ...
      console.warn("AgentHalo: settings:open-codex-pets-dir failed:", err && err.message);
  ...
  async function importCodexPetZip(event) {
    ...
    try {
      ...
      console.warn("AgentHalo: Codex Pet zip picker failed:", err && err.message);
      return { status: "error", message: (err && err.message) || String(err) };
    }
  ...
  async function importCodexPetZipFile(zipPath) {
    try {
      ...
      console.warn("AgentHalo: Codex Pet zip import failed:", err && err.message);
      return { status: "error", message: (err && err.message) || String(err) };
    }
  }
  ...
  async function removeCodexPet(themeId) {
    ...
    const displayName = localizeField(meta.name, getLang())
      || target.marker.sourcePetId
      || themeId;
    ...
    try {
      ...
      console.warn("AgentHalo: Codex Pet removal failed:", err && err.message);
      return { status: "error", message: (err && err.message) || String(err) };
    }
  }
