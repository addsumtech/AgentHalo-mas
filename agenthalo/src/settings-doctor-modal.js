...
(function initSettingsDoctorModal(root) {
  ...
  function connectionStatusLabel(core, test) {
    ...
    const map = {
      "recent-activity": "doctorConnectionRecent",
  ...
  function connectionDetailText(core, test) {
    ...
    if (test && test.status === "recent-activity") return t(core, "doctorConnectionRecentHint").replace("{agents}", (test.agents || []).join(", "));
    if (!test) return t(core, "doctorConnectionQuietHint");
  ...
  function renderLocalServerCheck(core, check, cls) {
    ...
    const detail = check.status === "pass" && check.runtime && check.runtime.port
      ? `127.0.0.1:${check.runtime.port}` : String(check.detail || "");
      ...
      return (
        `<div class="doctor-check-row ${cls}">` +
          `<div class="doctor-check-main${fixButton ? " with-action" : ""}">` +
            ...
            fixButton +
          `</div>` +
          (check.detail && check.status !== "pass" ? `<div class="doctor-check-detail">${escape(core, check.detail)}</div>` : "") +
          repairFeedback +
        `</div>`
      );
    }).join("");
  ...
  function renderConnectionTest(core) {
    const test = state.lastResult && state.lastResult.connectionActivity || state.connectionTest;
  ...
  function renderSidebarIndicator(sidebar, core) {
    ...
    item.className = "soft-btn doctor-indicator";
    item.type = "button";
    item.innerHTML =
