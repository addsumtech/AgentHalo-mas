...
// AgentHalo weren't installed — never answer allow/deny on the user's behalf.
...
function handlePermissionPost(req, res, options) {
  ...
  // Buffers, not a string: `body += chunk` decodes each chunk on its own, so a
  // multi-byte character split across a TCP read became two replacement
  // characters. A large tool_input is guaranteed to arrive in several chunks,
  // and plan-review echoes its input back to the agent verbatim. Byte counting
  // stays on the raw chunks because the cap below is a byte cap.
  const bodyChunks = [];
  let bodySize = 0;
  let tooLarge = false;
  req.on("data", (chunk) => {
  ...
  req.on("end", () => {
    ...
    // JSON.parse("null") succeeds and returns null, which is a legal document
    // but not a permission request. Several field reads below sit above the
    // try block that guards the rest of the handler, so a null body threw out
    // of the request handler with no uncaughtException handler to catch it and
    // took the whole app down. Arrays and scalars are rejected for the same
    // reason: nothing downstream treats them as a request.
    ...
    try {
      ...
      if (isOpencodeFamily(agentId)) {
        ...
        // channel AgentHalo uses to send the decision back to the plugin,
      ...
      // AgentHalo intentionally does NOT show a permission bubble for agy. If a
      ...
      // permission prompt". Keep every fallback as 204/no-decision so AgentHalo
      ...
      // fallback 204/no-decision so AgentHalo never denies tools on cleanup or
      ...
      if (agentId === "zcode") {
        ...
        // (the per-agent gate) is the stronger opt-out that keeps AgentHalo fully
      ...
      // continue native flow" (Phase 0 §3, locked). Every AgentHalo path here
      ...
      // Pi is state-only in AgentHalo. Current extensions never POST /permission.
      ...
      // instead of turning AgentHalo fallback into a terminal confirmation prompt.
      if (agentId === "pi") {
        const toolName = typeof data.tool_name === "string" && data.tool_name ? data.tool_name : "unknown";
        if (ctx.doNotDisturb) {
          recordRequestHookEvent.droppedByDnd();
      ...
      // approval/request. A 204 means no AgentHalo decision; the plugin calls
      ...
      const sessionIdentity = resolvePermissionSession(data.session_id, "default");
      const sessionId = sessionIdentity.sessionId;
      // Tag the permEntry with the source agent. AgentHalo's HTTP permission
      ...
      if (shouldBypassCCBubble(ctx, interaction, permAgentId)) {
        ...
        // a stronger opt-out that keeps AgentHalo fully out of that agent's loop —
