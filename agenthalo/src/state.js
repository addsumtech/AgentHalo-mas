...
function resolvePidReachable(existing, agentPid, sourcePid, host) {
  // A pid only means something in the namespace that issued it. WSL hooks
  // report the pid from inside the distro while AgentHalo runs on Windows, so
  // an unrelated host process with the same number latches this true and never
  // clears it — the stale sweep then deletes a live session the moment that
  // stranger exits. SSH remotes avoid this only by accident (their hooks send
  // no pid), so age out every hosted session on time instead.
...
// Every removal path runs the same teardown. Three of the six callers used to
// rely on this helper alone, which cancelled only the completion timers, while
// the SessionEnd and stale-delete paths did the rest by hand. So dismissing a
// card, evicting the oldest session, or a subagent stopping with no resume all
// left behind the things those two cleaned up: a per-session "allow for this
// session" grant that then re-bound when the same id came back, and a Kimi hold
// whose bubble kept the pet pinned on notification.
function deleteSessionWithCompletionCleanup(sessionId, reason, options = {}) {
  const session = sessions.get(sessionId);
  cancelCompletionDebounce(sessionId, reason);
  cancelClaudeTranscriptCompletionProbe(sessionId, reason);
  const agentId = (session && session.agentId) || options.agentIdFallback || null;
  if (agentId === "codex") cancelCodexExitProbe(sessionId, reason);
  if (agentId === "kimi-cli") disposeKimiSessionState(sessionId, reason);
  // A subagent-scoped SessionEnd retires one child, not the parent's consent.
  if (
    options.keepAutomationGrant !== true
    && agentId
    && typeof ctx.onSessionAutomationLifecycleEnd === "function"
  ) {
    ctx.onSessionAutomationLifecycleEnd({ agentId, sessionId, reason });
  }
...
// Trae stores the session title server-side, so AgentHalo derives it from the
...
function updateSession(sessionId, state, event, opts = {}) {
  ...
  if (event === "SessionEnd") {
    ...
    deleteSessionWithCompletionCleanup(sessionId, "session-end", {
      // The grant belongs to the parent, so a subagent-scoped end leaves it be.
      keepAutomationGrant: !!subagentId || !!subagentType,
      // A SessionEnd can arrive for a session we never stored.
      agentIdFallback: srcAgentId,
    });
...
function startStartupRecovery() {
  // The caller runs on every renderer did-finish-load, not only at first boot,
  // so re-arming without clearing orphaned a timer per reload and let the older
  // one fire early and cut the newer recovery window short.
