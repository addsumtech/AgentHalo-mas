...
describe("state-session-snapshot builder", () => {
  ...
  it("hides Claude-owned Codex exec children without changing lifecycle or permission metadata", () => {
    const cwd = "/private/tmp/claude-501/-Users-example/11111111-2222-4333-8444-555555555555/scratchpad/rich/A";
    const child = session("working", { agentId: "codex", codexSource: "exec", codexOriginator: "codex_exec", cwd, sourcePid: 123, headless: false });
    const sessions = new Map([
      ["child", child],
      ["headless-child", session("working", { headless: true })],
      ["main", session("working")],
      ["manual-exec", session("working", { agentId: "codex", codexSource: "exec", cwd: "/tmp/A" })],
      ["interactive", session("working", { agentId: "codex", codexSource: "cli", cwd })],
    ]);
    const snapshot = buildSessionSnapshot(sessions);
    const entry = snapshot.sessions.find(item => item.id === "child");
    assert.strictEqual(entry.hiddenFromSessionList, true);
    assert.strictEqual(entry.hiddenFromHud, true);
    assert.strictEqual(entry.canFocus, false);
    assert.strictEqual(entry.headless, false);
    assert.strictEqual(child.headless, false);
    assert.strictEqual(child.state, "working");
    assert.deepStrictEqual(snapshot.orderedIds.slice().sort(), ["interactive", "main", "manual-exec"]);
    assert.deepStrictEqual(snapshot.menuOrderedIds.slice().sort(), ["interactive", "main", "manual-exec"]);
    assert.strictEqual(snapshot.hudTotalNonIdle, 3);
    assert.ok(!snapshot.groups.some(group => group.ids.includes("child")));
    assert.notStrictEqual(sessionSnapshotSignature(snapshot), sessionSnapshotSignature(buildSessionSnapshot(new Map([
      ["child", { ...child, cwd: "/tmp/A" }], ...[...sessions].filter(([id]) => id !== "child"),
    ]))));
  });
  ...
  it("keeps Codex Desktop sessions that share one agent process visible in HUD", () => {
    ...
    assert.strictEqual(snapshot.sessions.find((entry) => entry.id === "codex:guardian").hiddenFromHud, true);
    assert.strictEqual(snapshot.hudTotalNonIdle, 2);
