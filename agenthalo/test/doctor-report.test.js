...
describe("doctor report redaction", () => {
  ...
  it("redacts configured AgentHalo app roots without consuming the rest of the path", () => {
    const out = redact([
...
describe("formatDiagnosticReport", () => {
  it("formats structured agent diagnostics into visible detail text", () => {
    const detail = formatAgentDetail({
      ...
      codexHookTrust: {
        ...
        value: "needs-review",
        detail: "2/2 AgentHalo Codex hook(s) need Codex /hooks review: PermissionRequest, Stop",
      },
      kiroScan: {
  ...
  it("formats Gemini supplementary diagnostics into visible detail text", () => {
    const detail = formatAgentDetail({
      detail: "Gemini hooks are disabled in settings.json; AgentHalo preserves this user setting and will not receive hook events",
      supplementary: {
        key: "gemini_hooks",
  ...
  it("formats summary and agent integration details", () => {
    ...
    assert.match(report, /# AgentHalo Diagnostic Report/);
  ...
  it("keeps the fallback file table when no HTTP event reached AgentHalo", () => {
    const report = formatDiagnosticReport({
      generatedAt: "2026-04-28T14:32:00.000Z",
      overall: { status: "warning", issueCount: 1 },
      checks: [],
      connectionTest: {
        status: "http-blocked",
        level: "warning",
        detail: "File activity changed, but no HTTP hook event reached AgentHalo.",
        events: [],
        fileActivity: [{
          agentId: "codex",
          source: "file-mtime",
