...
describe("CodexLogMonitor", () => {
  ...
  it("ends an archived rollout while retaining its read position for unarchive", () => {
    ...
    monitor = new CodexLogMonitor(makeConfig(tmpDir), (...args) => events.push(args), {
      archivedDir,
      onSessionArchived: (sid) => ended.push(sid),
    });
    monitor._pollFile(file, TEST_FILENAME);
    const beforeArchive = events.length;
    fs.renameSync(file, path.join(archivedDir, TEST_FILENAME));
    monitor._poll();
    monitor._poll();
    assert.deepStrictEqual(ended, [EXPECTED_SID]);
    assert.strictEqual(monitor.isSessionArchived(EXPECTED_SID), true);
    assert.strictEqual(monitor._tracked.has(file), false);
    assert.strictEqual(events.length, beforeArchive);

    fs.renameSync(path.join(archivedDir, TEST_FILENAME), file);
    monitor._poll();
    assert.strictEqual(monitor.isSessionArchived(EXPECTED_SID), false);
    assert.strictEqual(events.length, beforeArchive, "unarchive must not replay old events");
    fs.appendFileSync(file, JSON.stringify({
      type: "event_msg", payload: { type: "task_started" },
    }) + "\n");
    monitor._poll();
    assert.strictEqual(events.length, beforeArchive + 1);
    assert.strictEqual(events.at(-1)[1], "thinking");
    assert.strictEqual(events.at(-1)[3].cwd, "/projects/foo");
  });

  it("excludes archived tasks at startup even if an active-path copy remains", () => {
    const file = path.join(dateDir, TEST_FILENAME);
    const archivedDir = path.join(tmpDir, "archived_sessions");
    fs.mkdirSync(archivedDir);
    fs.writeFileSync(file, '{"type":"event_msg","payload":{"type":"task_started"}}\n');
    fs.copyFileSync(file, path.join(archivedDir, TEST_FILENAME));
    const activeName = uniqueRolloutName(1);
    fs.writeFileSync(path.join(dateDir, activeName),
      '{"type":"event_msg","payload":{"type":"task_started"}}\n');
    const events = [];
    const ended = [];
    monitor = new CodexLogMonitor(makeConfig(tmpDir), (...args) => events.push(args), {
      archivedDir, onSessionArchived: (sid) => ended.push(sid),
    });
    monitor._poll();
    assert.deepStrictEqual(ended, [EXPECTED_SID]);
    assert.strictEqual(events.some(([sid]) => sid === EXPECTED_SID), false);
    assert.strictEqual(events.some(([, state]) => state === "thinking"), true);
    assert.strictEqual(monitor._recoverStalePendingUserInput(file, TEST_FILENAME), null);
  });

  it("does not mistake a transient archive read error for unarchiving", () => {
    const archivedDir = path.join(tmpDir, "archived_sessions");
    fs.mkdirSync(archivedDir);
    fs.writeFileSync(path.join(archivedDir, TEST_FILENAME), "");
    monitor = new CodexLogMonitor(makeConfig(tmpDir), () => {}, { archivedDir });
    monitor._poll();
    const originalStat = fs.statSync;
    try {
      fs.statSync = (file, ...args) => {
        if (file === archivedDir) throw Object.assign(new Error("unreadable"), { code: "EACCES" });
  ...
  it("cleans up a silent exited exec only after two valid inventories, and allows it to resume", () => {
    const file = path.join(dateDir, TEST_FILENAME);
    fs.writeFileSync(file, [
    ...
    const events = [];
    monitor = new CodexLogMonitor(makeConfig(tmpDir), (...args) => events.push(args), {
      platform: "darwin", onSessionClosed: (...args) => ended.push(args),
    });
    monitor._pollFile(file, TEST_FILENAME);
    const old = new Date(Date.now() - 60000);
    fs.utimesSync(file, old, old);
    let scan = 10000, valid = true, owner = 123;
    monitor._findCodexWriterPid = () => {
      monitor._writerPidCacheAt = scan;
      monitor._writerPidScanValid = valid;
      return owner;
    };
    monitor._reconcileExitedExecSessions();
    assert.equal(ended.length, 0, "a long live call is retained");
    owner = null;
    monitor._reconcileExitedExecSessions();
    monitor._reconcileExitedExecSessions();
    assert.equal(ended.length, 0, "a cached scan is not fresh evidence");
    scan += 5000;
    valid = false;
    monitor._reconcileExitedExecSessions();
    assert.equal(ended.length, 0, "failed inventory does not prove exit");
    valid = true;
    monitor._reconcileExitedExecSessions();
    scan += 5000;
    ...
    fs.utimesSync(file, old, old);
    monitor._reconcileExitedExecSessions();
    assert.equal(ended.length, 0, "changed file resets the evidence");
    scan += 5000;
    monitor._reconcileExitedExecSessions();
    monitor._reconcileExitedExecSessions();
    ...
    monitor._pollFile(file, TEST_FILENAME);
    assert.equal(monitor._tracked.get(file).execEnded, false);
  ...
  it("never applies exec-exit cleanup to desktop tasks or an in-progress replay", () => {
    ...
    monitor = new CodexLogMonitor(makeConfig(tmpDir), () => {}, {
      platform: "darwin", onSessionClosed: () => assert.fail("must retain task"),
    });
    monitor._pollFile(file, TEST_FILENAME);
    monitor._findCodexWriterPid = () => assert.fail("must not inspect unrelated task");
    monitor._reconcileExitedExecSessions();
    monitor._tracked.get(file).codexSource = "exec";
    monitor._replayWork.set(file, {});
    monitor._reconcileExitedExecSessions();
    monitor._replayWork.clear();
  });
