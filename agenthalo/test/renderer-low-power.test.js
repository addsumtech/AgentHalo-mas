...
function createRendererHarness(options = {}) {
  ...
  const electronAPI = new Proxy({}, {
    get(_target, prop) {
      ...
      if (name === "isSystemAudioMuted") return async () => !!options.systemAudioMuted;
...
const source = `${readNormalized(path.join(__dirname, "..", "src", "sound-playback.js"))}
${readNormalized(ACCESSORY_LAYOUT)}
...
describe("renderer directional drag reactions (#620)", () => {
  ...
  it("warns once and falls back to a media swap when the universal marker is unavailable", () => {
    ...
    assert.deepStrictEqual(harness.warnings, [
      "AgentHalo: Codex Pet visual bridge unavailable (v1 marker missing); using a normal media swap.",
    ]);
  ...
  it("bounds marker and contentDocument failures without replacing the active object", () => {
    ...
    assert.strictEqual(harness.api.activeSwapToken, token);
    assert.deepStrictEqual(harness.warnings, [
      "AgentHalo: directional drag bridge unavailable (v1 marker missing); keeping the fallback direction.",
    ]);
  ...
  it("warns once when a shared directional wrapper is forced onto the image channel", () => {
    ...
    assert.deepStrictEqual(harness.warnings, [
      "AgentHalo: directional drag bridge unavailable (non-object media channel); keeping the fallback direction.",
    ]);
  });
...
describe("renderer sound preload and warmup", () => {
  it("does not create or play audio while the system is muted, including previews", async () => {
    const harness = createRendererHarness({ systemAudioMuted: true });
    await harness.electronHandlers.onPlaySound({ url: "file:///complete.mp3", volume: 1 });
    await harness.electronHandlers.onPlaySound({ url: "file:///confirm.mp3?_t=1", volume: 1 });
    assert.strictEqual(harness.audioInstances.length, 0);
  });
  ...
  it("does not reload a cached sound object on playback", async () => {
    const harness = createRendererHarness();
    const preload = harness.electronHandlers.onPreloadSounds;
    ...
    await playSound({ url: "file:///complete.mp3", volume: 1 });
