...
test("recap tab is loaded before the Settings renderer and is a secondary preference page", () => {
  ...
  assert.match(renderer, /recap: "general"/);
});
...
test("known agent colors match the approved palette and fallbacks are deterministic", () => {
  ...
  assert.equal(color("workbuddy"), "var(--recap-agent-workbuddy)");
  assert.equal(new Set([color("claude-code"), color("codex"), color("workbuddy")]).size, 3);
  assert.equal(color("custom-example"), color("custom-example"));
  assert.match(color("custom-example"), /^var\(--recap-agent-fallback-\d+\)$/);
});


test("every built-in agent has a distinct predefined color in light and dark mode", () => {
  const context = {};
  context.globalThis = context;
  vm.runInNewContext(fs.readFileSync(path.join(SRC, "settings-tab-recap.js"), "utf8"), context);
  const color = context.ClawdSettingsTabRecap.__test.agentColorToken;
  const agents = require("../agents/registry").getAllAgents();
  const tokens = agents.map((agent) => color(agent.id));
  assert.equal(new Set(tokens).size, agents.length);
  const css = fs.readFileSync(path.join(SRC, "settings.css"), "utf8");
  const light = [], dark = [];
  for (const token of tokens) {
    assert.ok(!token.includes("fallback"), token);
    const name = token.slice(4, -1);
    const values = [...css.matchAll(new RegExp(name + ":\\s*(#[0-9a-fA-F]{6});", "g"))].map((m) => m[1]);
    assert.equal(values.length, 2, name);
    light.push(values[0]); dark.push(values[1]);
  }
  assert.equal(new Set(light).size, agents.length);
  assert.equal(new Set(dark).size, agents.length);
});
