...
describe("session HUD layout", () => {
  ...
  it("shows all seven sessions with state labels", () => {
    const sessions = [];
    const orderedIds = [];
    for (let i = 0; i < 7; i++) {
    ...
    assert.strictEqual(expanded.length, 7);
    assert.strictEqual(folded.length, 0);
    assert.strictEqual(rowCount, 7);
  });

  it("shows all five sessions when state labels are hidden", () => {
    const sessions = [];
    const orderedIds = [];
    for (let i = 0; i < 5; i++) {
      sessions.push(mkSession(`s${i}`));
  ...
  it("respects orderedIds for picking the expanded set (most recent first)", () => {
    ...
    assert.deepStrictEqual(expanded.map((s) => s.id), ["newest", "middle", "old", "oldest"]);
    assert.deepStrictEqual(folded.map((s) => s.id), []);
  });
