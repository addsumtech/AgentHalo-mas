...
test("built-in themes resolve completion and confirmation to distinct bundled files", () => {
  ...
  assert.strictEqual(themes.length, 20);
  for (const { id: themeId } of themes) {
    ...
    assert.strictEqual(fileURLToPath(completeUrl), path.join(ROOT, "themes", themeId, "sounds", "complete.mp3"));
    assert.strictEqual(fileURLToPath(confirmUrl), path.join(ROOT, "themes", themeId, "sounds", "confirm.mp3"));
    assert.ok(fs.existsSync(fileURLToPath(completeUrl)));
    assert.ok(fs.existsSync(fileURLToPath(confirmUrl)));
