...
describe("applyUpdate", () => {
  ...
  it("allows both visibility preferences to be disabled sequentially", async () => {
    ...
    assert.strictEqual(r1.status, "ok");
    // The second switch is independent of the first.
    const r2 = await ctrl.applyUpdate("showDock", false);
    assert.strictEqual(r2.status, "ok");
    assert.strictEqual(ctrl.get("showDock"), false);
  });
...
describe("applyBulk", () => {
  ...
  it("allows both visibility preferences to be disabled in one update", async () => {
    const ctrl = createSettingsController({
      prefsPath: makeTempPath(),
      loadResult: { snapshot: { ...prefs.getDefaults(), showTray: true, showDock: true }, locked: false },
    });
