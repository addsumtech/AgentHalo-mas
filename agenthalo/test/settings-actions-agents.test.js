...
test("settings agent integration commands share a serialization lock", () => {
  ...
  assert.strictEqual(agentCommands.authorizeAgentConfigDir.lockKey, "agentIntegration");
  ...
  assert.strictEqual(agentCommands.prepareWebBridge.lockKey, "agentIntegration");
  assert.strictEqual(agentCommands.ensureWebBridge.lockKey, "agentIntegration");
});
