...
describe("agent-runtime-main", () => {
  it("preserves WorkBuddy work across late SessionStart and drops archived hooks", () => {
    ...
    runtime.updateSessionFromServer("wb-order", "idle", "SessionStart", { ...opts, sourcePid: 42 });
    assert.equal(state.sessions.get("wb-order").state, "thinking");
    assert.equal(state.sessions.get("wb-order").sourcePid, 42);
    runtime.updateSessionFromServer("wb-order", "attention", "Stop", opts);
    const completed = state.sessions.get("wb-order").state;
    runtime.updateSessionFromServer("wb-order", "idle", "SessionStart", opts);
    assert.equal(state.sessions.get("wb-order").state, completed);
    state.updateSession("wb-order", "idle", "SessionEnd", opts);
    closed = true;
  ...
  it("removes an archived local Codex card and rejects late hooks without affecting remote sessions", () => {
    const instances = [];
    const { state } = makeRealStateHarness();
    const sid = "codex:archived-thread";
    const localId = localSessionKey(sid);
    const remoteId = makeSessionKey({ profileId: "remote", rawSessionId: sid });
    const otherId = localSessionKey("codex:still-active");
    const cleared = [];
    const runtime = createAgentRuntimeMain({
      ...
      clearCodexNotifyBubbles: (...args) => cleared.push(args),
      clearCodexUserInputBubbles: (...args) => cleared.push(args),
    });
    const monitor = runtime.startCodexLogMonitor();
    let archived = false;
    monitor.isSessionArchived = (rawId) => archived && rawId === sid;
    const localOpts = {
      agentId: "codex", profileId: "local", rawSessionId: sid,
      hookSource: "codex-official", agentPid: process.pid,
    };
    runtime.updateSessionFromServer(localId, "working", "UserPromptSubmit", localOpts);
    state.updateSession(otherId, "working", "UserPromptSubmit", {
      agentId: "codex", profileId: "local", agentPid: process.pid,
    });
    assert.ok(state.sessions.has(localId));
    archived = true;
    monitor.options.onSessionArchived(sid);
    assert.strictEqual(state.sessions.has(localId), false);
    assert.strictEqual(state.sessions.has(otherId), true);
    assert.ok(cleared.some((args) => args[0] === localId && args.includes("codex-session-archived")));
    assert.strictEqual(runtime.updateSessionFromServer(localId, "working", "PreToolUse", localOpts), false);
    assert.strictEqual(state.sessions.has(localId), false);
    runtime.updateSessionFromServer(remoteId, "working", "UserPromptSubmit", {
      ...localOpts, profileId: "remote", host: "example-host",
    });
    assert.strictEqual(state.sessions.has(remoteId), true);
    archived = false;
    runtime.updateSessionFromServer(localId, "thinking", "UserPromptSubmit", localOpts);
    assert.strictEqual(state.sessions.has(localId), true);
    runtime.cleanup();
    state.clearSessionsByAgent("codex");
  });
