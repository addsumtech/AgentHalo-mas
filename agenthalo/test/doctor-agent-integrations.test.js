...
describe("checkAgentIntegrations", () => {
  ...
  it("turns Gemini ok into warning when hooksConfig.enabled=false", () => {
    ...
    assert.strictEqual(detail.status, "not-connected");
    assert.strictEqual(detail.level, "warning");
    assert.strictEqual(detail.detail, "Gemini hooks are disabled in settings.json; AgentHalo preserves this user setting and will not receive hook events");
    assert.deepStrictEqual(detail.supplementary, {
      key: "gemini_hooks",
  ...
  it("does not offer automatic repair when Qwen hooks are disabled globally", () => {
    ...
    assert.strictEqual(detail.status, "not-connected");
    assert.strictEqual(detail.level, "warning");
    assert.strictEqual(detail.detail, "Qwen Code hooks are disabled in settings.json; AgentHalo preserves this user setting and will not receive hook events");
    assert.deepStrictEqual(detail.supplementary, {
      key: "qwen_hooks",
      value: "disabled-global",
  ...
  it("warns without Fix when ZCode hooks are explicitly disabled globally", () => {
    ...
    assert.strictEqual(
      detail.detail,
      "ZCode config-file hooks are disabled globally; AgentHalo preserves hooks.enabled=false and will not receive hook events"
    );
    assert.deepStrictEqual(detail.supplementary, {
      key: "zcode_hooks",
      value: "disabled-global",
  ...
  it("warns without Fix when a managed ZCode event hook has enabled:false", () => {
    ...
    assert.deepStrictEqual(detail.supplementary, {
      ...
      detail: "AgentHalo hooks have enabled=false for: PreToolUse",
  ...
  it("reports a foreign PermissionRequest hook as a conflict without offering a Fix", () => {
    const descriptor = zcodeDescriptor();
    const config = zcodeHooksConfig();
    // Replace AgentHalo's entry with a user's own security hook.
    ...
    // A Fix would re-register AgentHalo's hook next to the foreign one and create
  ...
  it("reports coexistence of AgentHalo and foreign PermissionRequest hooks as a conflict", () => {
    const descriptor = zcodeDescriptor();
    const config = zcodeHooksConfig();
  ...
  it("warns and offers repair when Qoder has no AgentHalo hook", () => {
    const descriptor = qoderDescriptor();
    writeJson(descriptor.configPath, { hooks: {} });
  ...
  it("does not offer automatic repair when Antigravity AgentHalo hooks are disabled", () => {
    const descriptor = antigravityDescriptor();
    writeAntigravityHooks(descriptor, {
      clawd: {
        enabled: false,
    ...
    assert.strictEqual(detail.status, "not-connected");
    assert.strictEqual(detail.detail, "Antigravity AgentHalo hooks are disabled in hooks.json; AgentHalo preserves this user setting and will not receive hook events");
    assert.deepStrictEqual(detail.supplementary, {
  ...
  // #544: Windows AgentHalo writes dual-field entries — commandWindows carries
  ...
  // array with no AgentHalo plugin in it.
...
describe("kimi legacy permission-mode supplement", () => {
  ...
  it("never masks a primary finding and skips when legacy carries no AgentHalo hooks", () => {
    const { descriptor, legacyDir, kimiCodeDir } = kimiDescriptor();
    ...
    // kimi-code healthy + legacy dir exists but has no AgentHalo hooks: the
