...
test("packaged macOS auto-start launches the exact bundle executable", () => {
  const calls = [];
  const hooksDir = "/tmp/AgentHalo.app/Contents/Resources/app.asar.unpacked/hooks";
  launchApp({
    platform: "darwin",
    hooksDir,
  ...
  assert.deepStrictEqual(calls, [
    {
      command: "/tmp/AgentHalo.app/Contents/MacOS/AgentHalo",
      args: [],
...
test("packaged macOS auto-start survives a renamed outer app bundle", () => {
  const calls = [];
  const appBundle = "/Applications/AgentHalo 2.app";
  ...
  assert.deepStrictEqual(calls, [{
    command: `${appBundle}/Contents/MacOS/AgentHalo`,
    args: [],
    options: { detached: true, stdio: "ignore" },
  }]);
});
...
}), "/tmp/Renamed.app/Contents/MacOS/AgentHalo");
});
...
test("detached app launch handles asynchronous spawn errors", () => {
  ...
  launchApp({
    platform: "darwin",
    hooksDir: "/tmp/AgentHalo.app/Contents/Resources/app.asar.unpacked/hooks",
    spawn() {
      return child;
    },
    onSpawnError(err) {
