...
describe("session HUD visual shell", () => {
  ...
  it("keeps the rounded card while switching to a bottom-biased shadow", () => {
    assert.match(sessionHudHtml, /\.hud\s*\{[\s\S]*border-radius:\s*12px;[\s\S]*\}/);
  ...
  it("places the pin in a separate toolbar without reserving space in task rows", () => {
    assert.match(sessionHudHtml, /\.hud\.has-pin\s+\.row\s*\{[\s\S]*padding-right:\s*12px;[\s\S]*\}/);
    assert.doesNotMatch(sessionHudHtml, /\.hud\.has-pin\s+\.row\s+\.right\s*\{[\s\S]*padding-right:/);
  });

  it("preserves task titles and keeps technical focus errors out of the HUD", () => {
    ...
    assert.match(sessionHudRenderer, /if \(canFocus\)/);
    assert.doesNotMatch(sessionHudRenderer, /sessionFeedbackText|focusUnavailableText|focus-unavailable|SESSION_ACTION_FEEDBACK_MS/);
  });

  it("uses two status labels and colors while retaining completion acknowledgement", () => {
    assert.match(sessionHudHtml, /--done:\s*var\(--idle\)/);
    assert.match(sessionHudHtml, /--interrupted:\s*var\(--idle\)/);
    assert.match(sessionHudRenderer, /t\("sessionBadgeRunning"\)/);
    assert.match(sessionHudRenderer, /t\("sessionBadgeIdle"\)/);
    assert.doesNotMatch(sessionHudRenderer, /STATE_CHIP_MAP|EVENT_CHIP_MAP|sessionRecovered/);
