...
test("settings-i18n.js distinguishes recovery, final port failure, and deployment identity in English and Chinese", () => {
  ...
  assert.match(code, /remoteSshErrSecureIdentityMissing:\s*"AgentHalo's secure Remote SSH deployment identity[^"]*not SSH key authentication/);
  ...
  assert.match(code, /remoteSshErrSecureIdentityMissing:\s*"AgentHalo 的安全 Remote SSH 部署身份[^"]*不是 SSH 私钥认证失败/);
});
