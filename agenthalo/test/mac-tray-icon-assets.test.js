...
describe("macOS tray Template assets", () => {
  ...
  it("keeps the halo and two eyes separated at menu-bar size", () => {
    const image = decodeRgbaPng(RUNTIME.normal2x);
    for (let y = 19; y <= 28; y += 1) {
      for (let x = 16; x <= 19; x += 1) {
        assert.strictEqual(image.alpha[y * image.width + x], 0, `eyes must remain distinct at ${x},${y}`);
      }
    }
  });
  ...
  it("keeps the mark readable and inside the 18pt logical slot", () => {
    for (const [key, file] of Object.entries(RUNTIME)) {
      ...
      assert.ok(bounds.maxX < image.width && bounds.maxY < image.height);
    }
  });
