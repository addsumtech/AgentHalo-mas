...
describe("legacy Cloudling regression fixture", () => {
  it("loads as schema v1 with trusted scripted SVG files scoped to the built-in theme", () => {
    ...
    for (const file of CLOUDLING_ASSETS) {
      assert.ok(
        fs.existsSync(path.join(__dirname, "fixtures", "legacy-app", "themes", "cloudling", "assets", file)),
  ...
  it("exposes temporary pointer bridge shims in the prototype idle assets", () => {
    for (const file of ["cloudling-idle.svg", "cloudling-mini-idle.svg"]) {
      const assetPath = path.join(__dirname, "fixtures", "legacy-app", "themes", "cloudling", "assets", file);
  ...
  it("lets low-power idle mode stop Cloudling idle RAF loops", () => {
    ...
    for (const { file, source } of checks) {
      const asset = fs.readFileSync(path.join(__dirname, "fixtures", "legacy-app", "themes", "cloudling", "assets", file), "utf8");
  ...
  it("uses a visible normal idle eye-follow range for bridge validation", () => {
    const asset = fs.readFileSync(
      path.join(__dirname, "fixtures", "legacy-app", "themes", "cloudling", "assets", "cloudling-idle.svg"),
      "utf8"
    );
  ...
  it("uses tuned mini idle eye-follow and distance scaling for bridge validation", () => {
    const asset = fs.readFileSync(
      path.join(__dirname, "fixtures", "legacy-app", "themes", "cloudling", "assets", "cloudling-mini-idle.svg"),
      "utf8"
    );
    const source = fs.readFileSync(
  ...
  it("lets mini sleep pre-mirror Zzz glyphs for left-edge mini mode", () => {
    const asset = fs.readFileSync(
      path.join(__dirname, "fixtures", "legacy-app", "themes", "cloudling", "assets", "cloudling-mini-sleep.svg"),
      "utf8"
    );
