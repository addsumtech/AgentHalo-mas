...
describe("WorkBuddy hook session_id filter (#618 / #648) — real subprocess", () => {
  ...
  it("does not let an auth_success notification interrupt a running task", () => {
    const result = runHook({hook_event_name: "Notification", notification_type: "auth_success",
      session_id: "workbuddy-auth-test", message: "auth_success: Test account"}, "expect-none");
    assert.strictEqual(result.status, 0); assert.strictEqual(result.stdout, "{}\n");
