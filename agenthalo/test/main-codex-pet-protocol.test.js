...
test("main wires agenthalo:// protocol dispatch through the Codex Pet importer", () => {
...
test("package metadata registers only the agenthalo protocol and exposes dev registration", () => {
  ...
  // clawd:// belongs to upstream Clawd on Desk; claiming it in the store
  // build's Info.plist would collide with that app.
  assert.deepStrictEqual(protocols.flatMap((entry) => entry.schemes || []), ["agenthalo"]);
