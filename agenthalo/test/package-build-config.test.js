  ...
  describe("Telegram legacy retirement packaging", () => {
    ...
    it("contains no retired scripts, prebuild hooks, or extraResources", () => {
      ...
      assert.deepEqual(pkg.build.extraResources, [
        ...
        {
          from: "../agenthalo-web-bridge/extension",
          to: "web-bridge-extension",
          filter: ["manifest.json", "background.js", "content.js", "conversation-url.js", "icons/**/*"],
        },
      ]);
    });

    it("packages the browser extension without its per-machine config", () => {
      const bridge = pkg.build.extraResources.find((entry) => entry && entry.to === "web-bridge-extension");
      assert.ok(bridge, "web bridge extension should be packaged");
      assert.ok(!bridge.filter.includes("config.js"), "per-machine ids must not ship inside the app");
    });
