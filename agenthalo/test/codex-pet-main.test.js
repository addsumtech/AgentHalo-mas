...
test("Codex Pet main helpers detect agenthalo protocol args case-insensitively", () => {
  const { extractClawdProtocolUrls } = createCodexPetMain.__test;
  assert.deepStrictEqual(
    extractClawdProtocolUrls([
      "AgentHalo://import-pet?url=https%3A%2F%2Fexample.test%2Fpet.json",
      "--flag",
      "https://example.test",
      // Upstream Clawd on Desk's scheme; the store build leaves it to that app.
...
test("Codex Pet import URLs queued before app ready do not flush until explicitly drained", async () => {
  ...
  try {
    ...
    runtime.enqueueImportUrl("agenthalo://import-pet?url=https%3A%2F%2Fexample.test%2Fpet.json");
    ...
    assert.deepStrictEqual(parseCalls, ["agenthalo://import-pet?url=https%3A%2F%2Fexample.test%2Fpet.json"]);
...
test("Codex Pet import queue ignores overlapping flush calls while the first drain is active", async () => {
  ...
  runtime.enqueueImportUrl("agenthalo://import-pet?url=https%3A%2F%2Fexample.test%2Fone.json");
  runtime.enqueueImportUrl("agenthalo://import-pet?url=https%3A%2F%2Fexample.test%2Ftwo.json");
  ...
  assert.deepStrictEqual(parseCalls, ["agenthalo://import-pet?url=https%3A%2F%2Fexample.test%2Fone.json"]);
  ...
  assert.deepStrictEqual(parseCalls, [
    "agenthalo://import-pet?url=https%3A%2F%2Fexample.test%2Fone.json",
    "agenthalo://import-pet?url=https%3A%2F%2Fexample.test%2Ftwo.json",
  ]);
