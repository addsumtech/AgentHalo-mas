...
describe("validate-theme.js CLI (real process, spawnSync)", () => {
  ...
  it("--assets given with no value is refused, not silently dropped", () => {
    ...
    const result = runValidateTheme([THEME, "--assets"]);
    assert.strictEqual(result.status, 1, result.stderr || result.stdout);
  ...
  it("--assets followed by another flag fails naming what it received", () => {
    ...
    const result = runValidateTheme([THEME, "--assets", "--verbose"]);
    assert.strictEqual(result.status, 1, result.stderr || result.stdout);
  ...
  it("--assets given more than once is refused", () => {
    ...
    const result = runValidateTheme([THEME, "--assets", assets, "--assets", assets]);
    assert.strictEqual(result.status, 1, result.stderr || result.stdout);
  ...
  it("an unknown option is refused, not ignored", () => {
    ...
    const result = runValidateTheme([THEME, "--verbose"]);
    assert.strictEqual(result.status, 1, result.stderr || result.stdout);
  ...
  it("a surplus positional argument is refused, not ignored", () => {
    ...
    const result = runValidateTheme([THEME, path.join(REPO_ROOT, "themes", "clawd")]);
    assert.strictEqual(result.status, 1, result.stderr || result.stdout);
  ...
  it("--assets pointing at a directory that does not exist says so", () => {
    ...
    const result = runValidateTheme([THEME, "--assets", missing]);
    assert.strictEqual(result.status, 1, result.stderr || result.stdout);
    assert.match(result.stderr, /--assets directory not found/);
  });
  ...
  it("--assets pointing at a file says so instead of blaming the theme", () => {
    ...
    const result = runValidateTheme([THEME, "--assets", path.join(THEME, "theme.json")]);
    assert.strictEqual(result.status, 1, result.stderr || result.stdout);
  ...
  it("--assets whose parent is a file reports the real error, not \"not found\"", {
    ...
    const result = runValidateTheme([THEME, "--assets", path.join(THEME, "theme.json", "nope")]);
    assert.strictEqual(result.status, 1, result.stderr || result.stdout);
  ...
  it("malformed objectChannelFiles produces a normal validation report instead of crashing", () => {
    for (const objectChannelFiles of [[42], 42, {}]) {
      const dir = mkTempThemeDir();
      const raw = JSON.parse(fs.readFileSync(path.join(THEME, "theme.json"), "utf8"));
      raw.schemaVersion = 2;
      raw.rendering = { ...(raw.rendering || {}), objectChannelFiles };
      fs.writeFileSync(path.join(dir, "theme.json"), JSON.stringify(raw), "utf8");
      fs.cpSync(path.join(THEME, "assets"), path.join(dir, "assets"), { recursive: true });
  ...
  it("control: an --assets value beginning with - is a path, not a flag", () => {
    ...
    fs.cpSync(path.join(THEME, "assets"), path.join(parent, "-dashed", "assets"), { recursive: true });
    const result = runValidateTheme([THEME, "--assets", "-dashed/assets"], { cwd: parent });
  ...
  it("control: a theme directory beginning with - is reachable after --", () => {
    ...
    const parent = mkTempThemeDir();
    fs.cpSync(THEME, path.join(parent, "-dashed-theme"), { recursive: true });
  ...
  it("--assets actually redirects the lookups, it is not just validated and dropped", () => {
    ...
    const redirected = runValidateTheme([THEME, "--assets", empty]);
    ...
    assert.strictEqual(runValidateTheme([THEME]).status, 0);
  });
  ...
  it("control: a well-formed --assets override still validates clean", () => {
    ...
    const result = runValidateTheme([THEME, "--assets", path.join(THEME, "assets")]);
    assert.strictEqual(result.status, 0, result.stderr || result.stdout);
  });
