...
test("main falls back before startup theme load when active Codex Pet theme is orphaned", () => {
  ...
  assert.ok(source.includes('_requestedThemeId = "halo";'));
...
test("settings exposes Codex Pet refresh and managed theme metadata", () => {
  ...
  assert.ok(tabSource.includes("themeRefreshThemes"));
  assert.ok(tabSource.includes("themeImportCompanionZip"));
  assert.ok(preloadSource.includes('importCompanionZip: () => ipcRenderer.invoke("settings:import-companion-zip")'));
