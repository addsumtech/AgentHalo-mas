...
describe("Kimi quota HTTPS client", () => {
  it("uses the fixed endpoint, GET, bearer auth, and truthful AgentHalo UA", async () => {
    ...
    assert.strictEqual(mock.calls[0].options.headers["User-Agent"], "AgentHalo/0.15.0 KimiQuota/experimental");
  ...
  it("sanitizes an unsafe app version before constructing User-Agent", () => {
    ...
    assert.strictEqual(client.userAgent, "AgentHalo/unknown KimiQuota/experimental");
  });
});
