...
// settings.html loads this before the tab modules; the General tab's volume
// row reads window.AgentHaloSoundPlayback at render time.
const SOUND_PLAYBACK_JS = path.join(SRC_DIR, "sound-playback.js");
// settings.html loads this before the tab modules; theme names and other
// author-supplied text are language-keyed objects resolved through it.
const LOCALIZED_FIELD_JS = path.join(SRC_DIR, "localized-field.js");
...
function loadSettingsI18nBundleForTest() {
  ...
  vm.runInContext(fs.readFileSync(path.join(SRC_DIR, "ui-color-theme.js"), "utf8"), context);
  vm.runInContext(fs.readFileSync(LOCALIZED_FIELD_JS, "utf8"), context);
...
function loadGeneralLanguageRowForTest({
  ...
  vm.runInContext(fs.readFileSync(SETTINGS_ANIM_OVERRIDES_MERGE, "utf8"), context);
  vm.runInContext(fs.readFileSync(SETTINGS_UI_CORE, "utf8"), context);
  vm.runInContext(fs.readFileSync(SOUND_PLAYBACK_JS, "utf8"), context);
...
describe("interface color picker", () => {
  it("saves the selected palette and patches its committed state without rebuilding General", async () => {
    const calls = [];
    const snapshot = makeGeneralSnapshot({ uiColorTheme: "moss" });
    const harness = loadGeneralTabForTest({ snapshot, settingsAPI: {
      update: async (key, value) => { calls.push({ key, value }); return { status: "ok" }; },
    } });
    harness.renderContent();
    const picker = harness.content.querySelector(".ui-color-theme-picker");
    const buttons = picker.querySelectorAll("button");
    assert.strictEqual(buttons.length, 5);
    const classic = buttons.find((button) => button.dataset.value === "classic");
    classic.eventListeners.click[0]({ currentTarget: classic });
    await new Promise((resolve) => setImmediate(resolve));
    assert.deepStrictEqual(calls, [{ key: "uiColorTheme", value: "classic" }]);
    const count = harness.getContentRenderCount();
    harness.core.ops.applyChanges({ changes: { uiColorTheme: "classic" }, snapshot: { ...snapshot, uiColorTheme: "classic" } });
    assert.strictEqual(harness.getContentRenderCount(), count);
    assert.strictEqual(harness.content.querySelector(".ui-color-theme-picker"), picker);
    assert.strictEqual(classic.attributes["aria-checked"], "true");
  });

  it("restores the saved palette when persistence rejects a change", async () => {
    const harness = loadGeneralTabForTest({ snapshot: makeGeneralSnapshot({ uiColorTheme: "moss" }), settingsAPI: {
      update: async () => ({ status: "error", message: "disk unavailable" }),
    } });
    harness.renderContent();
    const buttons = harness.content.querySelector(".ui-color-theme-picker").querySelectorAll("button");
    const rose = buttons.find((button) => button.dataset.value === "rose");
    rose.eventListeners.click[0]({ currentTarget: rose });
    await new Promise((resolve) => setImmediate(resolve));
    assert.strictEqual(rose.attributes["aria-checked"], "false");
    assert.strictEqual(buttons.find((button) => button.dataset.value === "moss").attributes["aria-checked"], "true");
  });
});
...
describe("settings renderer browser environment", () => {
  ...
  it("keeps preferences compact without changing saved advanced values", () => {
    const snapshot = makeGeneralSnapshot({ permissionAutomationMode: "off", sessionStaleMs: 123000, showTray: true, showDock: false });
  ...
  it("retains the upstream contributor attribution in the source bundle", () => {
    const i18nBundle = loadSettingsI18nBundleForTest();
    for (const login of VERIFIED_GITHUB_CONTRIBUTORS) {
      assert.ok(i18nBundle.CONTRIBUTORS.includes(login), `Source credits should include ${login}`);
    }
  });

  it("presents AgentHalo feedback separately from upstream credit without update controls", async () => {
    const opened = [];
    const harness = loadAboutTabForTest({
      ...
      openExternalSafe: (url) => opened.push(url),
    });
    await new Promise((resolve) => setImmediate(resolve));
  ...
  it("wires Clawd Doctor through Settings with Step 2 connection actions", () => {
    ...
    assert.ok(fs.readFileSync(path.join(SRC_DIR, "settings-tab-agents.js"), "utf8").includes("ClawdSettingsDoctorModal.renderSidebarIndicator"));
    ...
    assert.ok(/\.doctor-modal\s*\{[\s\S]*width:\s*min\(680px,\s*100%\);[\s\S]*max-height:\s*calc\(100vh \/ var\(--clawd-text-zoom, 1\) - 32px\);/.test(css));
    assert.ok(/\.doctor-modal\s*\{[\s\S]*gap:\s*12px;[\s\S]*padding:\s*20px;/.test(css));
    ...
    assert.ok(/\.doctor-check-list\s*\{[^}]*overflow-y:\s*auto;/.test(css));
    assert.ok(/\.doctor-check-row\s*\{[^}]*padding:\s*11px 12px;/.test(css));
    ...
    assert.ok(/\.doctor-close:hover\s*\{[\s\S]*background:\s*color-mix\(in srgb, var\(--accent\) 10%, transparent\);[\s\S]*transform:\s*scale\(1\.04\);/.test(css));
  ...
  it("renders the Settings language picker as a dropdown over all supported langs", () => {
    ...
    assert.ok(/\.language-picker-option\.selected\s*\{[\s\S]*color:\s*var\(--accent-text, var\(--accent\)\);/.test(pickerCss));
  ...
  it("explains when the pet is too large to choose an activity area", async () => {
    ...
    assert.strictEqual(
      toasts[0].message,
      "AgentHalo is larger than this display's work area. Reduce the pet size before choosing an activity area.",
    );
    assert.strictEqual(toasts[0].options.error, true);
  });


  it("uses collapsible Session HUD options and a single visible sound control", () => {
    const generalSource = fs.readFileSync(path.join(SRC_DIR, "settings-tab-general.js"), "utf8");
    const css = fs.readFileSync(SETTINGS_CSS, "utf8");
    const i18nSource = fs.readFileSync(SETTINGS_I18N, "utf8");
    ...
    assert.ok(/\.session-hud-collapsible \.collapsible-summary-chip\s*\{[\s\S]*max-width:\s*min\(280px,\s*100%\);/.test(css));
  ...
  it("stacks wide General controls from their zoom-corrected card width", () => {
    ...
    assert.match(css, /\.quota-ring-collapsible \.settings-option-list,\s*\.sound-settings-group\s*\{\s*container-type:\s*inline-size;/s);
  ...
  it("describes notification bubble seconds as an auto-close upper bound instead of a guaranteed visible duration", () => {
    ...
    assert.ok(i18nSource.includes("最长停留时间"));
    assert.ok(i18nSource.includes("任务状态变化时可能提前关闭"));
  ...
  it("renders the macOS menu bar and Dock recovery switches independently in all four states", () => {
    const cases = [
      { showTray: true, showDock: false, trayDisabled: false, dockDisabled: false },
      { showTray: true, showDock: true, trayDisabled: false, dockDisabled: false },
      { showTray: false, showDock: true, trayDisabled: false, dockDisabled: false },
  ...
  it("patches committed macOS visibility changes in place", async () => {
    const updateCalls = [];
    const initialSnapshot = makeGeneralSnapshot({ showTray: true, showDock: false });
    const harness = loadGeneralTabForTest({
      platform: "MacIntel",
    ...
    assert.strictEqual(dock.getAttribute("aria-disabled"), undefined);
    assert.strictEqual(dock.tabIndex, 0);
  });

  it("allows disabling the last visible macOS entry point", async () => {
    const updateCalls = [];
    const harness = loadGeneralTabForTest({
      platform: "MacIntel",
    ...
    await Promise.resolve();
    await Promise.resolve();
    assert.deepStrictEqual(updateCalls, [{ key: "showDock", value: false }]);
  });
  ...
  it("exposes exactly one sound switch and the volume slider without expansion", () => {
    const harness = loadGeneralTabForTest({ snapshot: makeGeneralSnapshot({ soundMuted: false }) });
    harness.renderContent();
    const group = harness.content.querySelector(".sound-settings-group");
    assert.ok(group);
    assert.equal(group.querySelectorAll(".switch").length, 1);
    assert.ok(group.querySelector(".volume-slider"));
    assert.equal(group.querySelector(".collapsible-group-header"), null);
  });

  it("keeps the single sound switch and volume slider synchronized in place", () => {
    ...
    const optionList = harness.content.querySelector(".sound-settings-group");
    assert.ok(soundSwitch);
    assert.ok(summary);
    assert.ok(volumeControl);
    assert.ok(volumeSlider);
    assert.ok(optionList);
    assert.strictEqual(optionList.children.length, 2);
    assert.ok(harness.getSwitchMeta("soundMuted").row.querySelector(".row-desc"));
    assert.strictEqual(summary.children.length, 1);
    assert.ok(summary.children[0].classList.contains("sound-header-switch"));
    assert.strictEqual(summary.children[0].attributes["aria-label"], "Enable sound effects");
    ...
    assert.strictEqual(summary.children[0].classList.contains("on"), false);
    assert.strictEqual(volumeSlider.disabled, true);
  });

  it("toggles sound once while the volume row remains visible", async () => {
    const updateCalls = [];
    const initialSnapshot = makeGeneralSnapshot({
      soundMuted: false,
      soundVolume: 1,
    ...
    const soundGroup = harness.content.querySelector(".sound-settings-group");
    assert.ok(headerSwitch);
    assert.ok(soundGroup.querySelector(".volume-slider"));
  ...
  it("keeps the single sound switch pending until the update completes", async () => {
    const updateCalls = [];
    let resolveUpdate = null;
    const initialSnapshot = makeGeneralSnapshot({
      soundMuted: false,
  ...
  it("rerenders General content for mixed non-patchable broadcasts", () => {
    ...
    const master = harness.getSwitch("freeRoam");
    const beforeRenderCount = harness.getContentRenderCount();
    harness.core.ops.applyChanges({
      changes: { freeRoam: true, lang: "zh" },
      snapshot: { ...initialSnapshot, freeRoam: true, lang: "zh" },
    });

    assert.strictEqual(harness.getContentRenderCount(), beforeRenderCount + 1);
    assert.notStrictEqual(harness.getSwitch("freeRoam"), master);
    assert.strictEqual(harness.getSwitch("freeRoam").classList.contains("on"), true);
  });
  ...
  it("provides a persisted collapsible Settings group helper with smart default collapse", () => {
    ...
    assert.ok(/\.collapsible-group:not\(\.collapsed\)\s*>\s*\.collapsible-group-header\s*>\s*\.collapsible-group-chevron\s*\{[\s\S]*transform:\s*translateX\(-6px\) rotate\(90deg\);[\s\S]*color:\s*var\(--accent-text, var\(--accent\)\);[\s\S]*opacity:\s*1;/.test(css));
  ...
  it("groups Theme cards and exposes one character import action in Settings", () => {
    ...
    assert.strictEqual(strings.en.themeImportUserThemeZip, "Import AgentHalo theme package (.zip)");
    ...
    assert.strictEqual(strings.zh.themeImportUserThemeZip, "导入 AgentHalo 主题包（.zip）");
  ...
  it("uses the shared SVG chevron treatment for Animation Overrides rows", () => {
    ...
    assert.ok(/\.anim-override-row\.expanded\s*>\s*\.anim-override-summary\s+\.anim-override-chevron\s*\{[\s\S]*transform:\s*translateX\(-6px\) rotate\(90deg\);[\s\S]*color:\s*var\(--accent-text, var\(--accent\)\);[\s\S]*opacity:\s*1;/.test(css));
  ...
  it("renders Animation Overrides theme actions in two intentional rows", () => {
    ...
    assert.strictEqual(strings.zh.animOverridesReplacementConfig, "自定义动画设置");
