...
function translations() {
  return {
    ...
    sessionMinAgo: "{n}m",
    sessionHrAgo: "{n}h",
    sessionBadgeRunning: i18n.en.sessionBadgeRunning,
    sessionBadgeIdle: i18n.en.sessionBadgeIdle,
    sessionLocal: "Local",
...
test("keeps background child tasks out of Dashboard and HUD cards", async () => {
  const main = session("main");
  const child = session("child", { hiddenFromSessionList: true, hiddenFromHud: true });
  const headless = session("headless", { headless: true });
  const dashboard = await loadDashboard([main, child, headless]);
  assert.strictEqual(byClass(dashboard.root, "card").length, 1);
  const hud = await loadHud([main, child, headless]);
  assert.strictEqual(byClass(hud.root, "row").length, 1);
});

test("Dashboard omits technical focus reasons and retains the available local folder action", async () => {
  ...
  assert.equal(byClass(root, "focus-unavailable-reason").length, 0);
...
test("HUD keeps task titles after an unavailable click and later snapshots", async () => {
  const harness = await loadHud([
    session("local"),
    session("remote", { sourceType: "ssh", host: "host" }),
    session("webui", { platform: "webui" }),
  ]);
  const rows = byClass(harness.root, "row-unfocusable");
  assert.deepStrictEqual(rows.map((row) => row.title), ["", "", ""]);
  await rows[0].dispatch("click");
  harness.pushSnapshot();
  assert.deepStrictEqual(byClass(harness.root, "title").map((el) => el.textContent), ["local", "remote", "webui"]);
  assert.equal(byClass(harness.root, "session-inline-feedback").length, 0);
  assert.equal(byClass(harness.root, "open-folder-button").length, 0);
  assert.deepStrictEqual(harness.openCalls, []);
});

test("HUD exposes only working and waiting labels for all internal states", async () => {
  const { root } = await loadHud([
    session("working", { badge: "running" }),
    session("done", { badge: "done" }),
    session("interrupted", { badge: "interrupted" }),
    session("idle", { badge: "idle" }),
  ]);
  assert.deepStrictEqual(byClass(root, "state-chip").map((el) => el.textContent), ["Working", "Waiting", "Waiting", "Waiting"]);
});
