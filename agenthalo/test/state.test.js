...
describe("cleanStaleSessions()", () => {
  ...
  it("a confirmed closed terminal clears its idle session before the age cutoff", () => {
    api = require("../src/state")(makeCtx({ processKill: makePidKill(new Set()) }));
    api.sessions.set("s1", rawSession("idle", {
      agentId: "claude-code",
      sourcePid: 9999,
  ...
  it("broadcasts removal when a terminal closes after its turn finished", () => {
    ...
    assert.strictEqual(broadcasts[1].sessions.some((s) => s.id === "s1"), false);
  });

  it("a confirmed terminal exit clears even a session without an ended badge", () => {
    api = require("../src/state")(makeCtx({
      processKill: makePidKill(new Set()),
      sessionHudCleanupDetached: true,
    }));
  ...
  it("detached ended Kimi auto-clear disposes notification state", () => {
    ...
    api.cleanStaleSessions();
    assert.strictEqual(api.sessions.size, 0);
    // The reason now names why the session went away rather than a fixed
    // "disposed" literal, so a stuck bubble in a log says what removed it.
    assert.deepStrictEqual(cleared, [{ id: "k1", reason: "stale-delete-source-exit" }]);
  });
...
describe("updateSession()", () => {
  ...
  it("never reads a hosted session's pid against the local process table", () => {
    // process.pid is by definition alive here — the exact coincidence that used
    // to latch a WSL session reachable and then delete it as "source-exit" when
    // the unrelated local process holding that number exited.
    update(api, { id: "wsl", host: "wsl:Ubuntu", sourcePid: process.pid, agentPid: process.pid });
    assert.strictEqual(api.sessions.get("wsl").pidReachable, false);

    update(api, { id: "ssh", host: "ssh:devbox", sourcePid: process.pid });
    assert.strictEqual(api.sessions.get("ssh").pidReachable, false);

    // Local sessions must keep using the check that makes exit detection work.
    update(api, { id: "local", sourcePid: process.pid });
    assert.strictEqual(api.sessions.get("local").pidReachable, true);
  });

  it("does not let a latched pidReachable survive a session becoming hosted", () => {
    update(api, { id: "s1", sourcePid: process.pid });
    assert.strictEqual(api.sessions.get("s1").pidReachable, true);

    update(api, { id: "s1", host: "wsl:Ubuntu", sourcePid: process.pid });
    assert.strictEqual(api.sessions.get("s1").pidReachable, false);
  });
...
describe("buildSessionSnapshot", () => {
  ...
  it("builds renderer-safe fields, groups, and both dashboard/menu orderings", () => {
    ...
    assert.doesNotThrow(() => JSON.stringify(snapshot));
    assert.deepStrictEqual(snapshot.orderedIds, ["error-local", "old-working"]);
    assert.deepStrictEqual(snapshot.menuOrderedIds, ["error-local", "old-working"]);
    ...
    assert.strictEqual(snapshot.lastSessionId, "error-local");
    assert.strictEqual(snapshot.lastTitle, "error-project");
  ...
  it("keeps headless sessions in raw data but excludes them from visible lists", () => {
    ...
    assert.deepStrictEqual(snapshot.orderedIds, ["interactive-active"]);
    assert.strictEqual(snapshot.sessions.length, 2);
    assert.strictEqual(snapshot.lastSessionId, "interactive-active");
