...
// The wardrobe is a theme capability, and no shipped character currently
// authors mouth attachments. Discover one rather than naming it, so this
// coverage comes back on its own the day a character authors them again.
function findMouthAccessoryTheme() {
  themeLoader.init(path.join(ROOT, "src"), app.getPath("userData"));
  for (const themeId of fs.readdirSync(path.join(ROOT, "themes")).sort()) {
    let theme;
    try {
      theme = themeLoader.loadTheme(themeId, { strict: true });
    } catch {
      continue;
    }
    const config = themeLoader.createThemeContext(theme).getRendererConfig();
    if (config.mouthAccessorySupported && config.idleFollowSvg) return { themeId, config };
  }
  return null;
}

async function assertProductionRendererBootsMouthObject() {
  const capable = findMouthAccessoryTheme();
  if (!capable) {
    console.log("skip: no shipped theme authors mouth accessory attachments");
    return;
  }
  const { themeId, config } = capable;
  config.idleDefaultVisual = config.idleFollowSvg;
  ...
  try {
    ...
    assert(result.file === config.idleFollowSvg, "production renderer did not settle on idle");
