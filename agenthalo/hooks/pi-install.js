...
function registerPiExtension(options = {}) {
  ...
  if (!parentExists && !hasPiCommand(options)) {
    if (!options.silent) {
      console.log("AgentHalo: Pi not found - skipping Pi extension registration");
    }
  ...
  if (extensionExists && !isManagedMarker(readJsonIfPresent(markerPath, fsImpl))) {
    if (!options.silent) {
      console.log(`AgentHalo: ${extensionDir} exists but is not Clawd-managed - skipping`);
    }
...
function unregisterPiExtension(options = {}) {
  ...
  if (!dirExists(extensionDir, fsImpl)) {
    if (!options.silent) console.log("AgentHalo: Pi extension is not installed");
    return { removed: false, skipped: true, reason: "missing", extensionDir };
  }
  if (!isManagedMarker(marker)) {
    if (!options.silent) console.log(`AgentHalo: ${extensionDir} is not Clawd-managed - skipping uninstall`);
  ...
  if (!options.silent) console.log(`AgentHalo: removed Pi extension from ${extensionDir}`);
