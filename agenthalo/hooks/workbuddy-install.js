...
function registerWorkBuddyHooks(options = {}) {
  ...
  if (!options.settingsPath && !fs.existsSync(workbuddyDir)) {
    if (!options.silent) console.log("AgentHalo: WorkBuddy config directory not found — skipping hook registration");
    ...
    for (const inactivePath of inactivePaths) {
      try {
        ...
        if (err && err.code === "INVALID_JSON") {
          ...
          if (!options.silent) console.warn(`AgentHalo: skipped invalid inactive WorkBuddy config ${inactivePath}`);
        } else {
          ...
          if (!options.silent) console.warn(`AgentHalo: could not clean inactive WorkBuddy config ${inactivePath}`);
        }
      }
    }
  }
