...
function registerOpenClawPlugin(options = {}) {
  ...
  if (!stateDirExists && !configFileExists && !getCommandAvailable()) {
    if (!options.silent) console.log("AgentHalo: OpenClaw not found - skipping OpenClaw plugin registration");
    return {
      installed: false,
      skipped: true,
      updated: false,
  ...
  if (!configFileExists) {
    ...
    if (!options.silent) {
      console.log(`AgentHalo: ${configPath} missing - skipping OpenClaw plugin registration`);
    }
    return {
      installed: false,
      skipped: true,
