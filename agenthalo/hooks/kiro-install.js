...
function syncClawdAgentFromBuiltin(filePath, options = {}) {
  ...
  if (!result.template) {
    ...
    if (!options.silent) {
      ...
      console.warn(`AgentHalo: kiro-cli template generation failed — ${fate}. Reason: ${result.error?.message || "unknown"}`);
    }
...
function registerKiroHooks(options = {}) {
  ...
  if (!fs.existsSync(agentsDir)) {
    if (!options.silent) console.log("AgentHalo: ~/.kiro/ not found — skipping Kiro hook registration");
  ...
  for (const file of jsonFiles) {
    ...
    try {
      ...
      if (!options.silent) console.warn(`AgentHalo: failed to process ${file}: ${err.message}`);
    }
  }
  ...
  try {
    ...
    if (!options.silent) console.warn(`AgentHalo: failed to sync ${CLAWD_AGENT_NAME}.json from ${BUILTIN_DEFAULT_AGENT}: ${err.message}`);
  }
  try {
    ...
    if (!options.silent) console.warn(`AgentHalo: failed to sync ${CLAWD_AGENT_NAME}.json: ${err.message}`);
  }

  if (!options.silent) {
    if (files.length > 0) {
      console.log(`AgentHalo: Kiro hooks injected into ${files.length} agent config(s): ${files.join(", ")}`);
      console.log(`  Added: ${totalAdded}, updated: ${totalUpdated}, skipped: ${totalSkipped}`);
    } else {
      console.log("AgentHalo: all Kiro agent configs already up to date");
    }
    console.log(`AgentHalo: use "kiro-cli --agent ${CLAWD_AGENT_NAME}" or run "/agent swap ${CLAWD_AGENT_NAME}" inside Kiro to enable hooks`);
  }
...
function unregisterKiroHooks(options = {}) {
  ...
  if (!fs.existsSync(agentsDir)) {
    if (!options.silent) console.log("AgentHalo: ~/.kiro/ not found - skipping Kiro hook cleanup");
