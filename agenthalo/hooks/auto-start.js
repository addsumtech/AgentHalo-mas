...
function resolveMacBundleExecutable(appBundle, options = {}) {
  ...
  if (!executableName) executableName = "AgentHalo";
  ...
  try {
    if (isPackaged) {
      if (isWin) {
        ...
        // exe:       <install>/AgentHalo.exe
        const installDir = path.resolve(hooksDir, "..", "..", "..");
        const exe = path.join(installDir, "AgentHalo.exe");
        spawnDetached(
          spawnProcess,
          exe,
          [],
        ...
        if (appImage) {
          ...
          const exe = path.posix.join(installDir, "agenthalo");
          spawnDetached(
            spawnProcess,
            exe,
            [],
