...
function registerKimiHooksAtTarget(target, options = {}) {
  ...
  if (!fs.existsSync(kimiDir)) {
    if (!options.silent) console.log(`AgentHalo: ${kimiDir} not found — skipping Kimi hook registration`);
...
function registerKimiHooks(options = {}) {
  ...
  for (const target of targets) {
    try {
      ...
      if (!options.silent) console.warn(`AgentHalo: Kimi hook sync failed for ${target.settingsPath}: ${err.message}`);
    }
  }
