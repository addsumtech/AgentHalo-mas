...
function run() {
  readStdinJson()
    .then((payload) => {
      ...
      if (!mapped || (hookName === "Notification" && payload.notification_type === "auth_success")) {
        // WorkBuddy emits auth_success immediately after a prompt. It is an
        // informational login event, not a request for attention or a turn end.
        finish(outLine);
        return;
      }
