...
// Ephemeral exec jobs have no interactive task to return to and may not write
// a transcript (so session_meta cannot identify these background workers).
function isCodexEphemeralCommandLine(commandLine) {
  const tokens = String(commandLine || "").match(/"[^"\n]*"|'[^'\n]*'|[^\s]+/g) || [];
  const args = tokens.map((token) => token.replace(/^["']|["']$/g, ""));
  if (!/(?:^|[\\/])codex(?:\.exe)?$/i.test(args[0] || "") || !["exec", "e"].includes(args[1])) return false;
  for (let index = 2; index < args.length; index++) {
    const arg = args[index];
    if (arg === "--" || !arg.startsWith("-")) return false;
    if (arg === "--ephemeral") return true;
    if (["-C", "--cd", "-c", "--config", "-m", "--model", "-s", "--sandbox", "-p", "--profile", "-o", "--output-last-message", "--color", "--output-schema", "--add-dir", "--enable", "--disable", "-i", "--image"].includes(arg)) index++;
  }
  return false;
}
...
async function runCodexHook(payload, options = {}) {
  ...
  const createAttemptResolver = (initialPreferredPort = null, processChainAttempt = null) => {
    ...
    const resolverOptions = {
      ...
      headlessCheck: isCodexEphemeralCommandLine,
      readRuntimeIdentity() {
