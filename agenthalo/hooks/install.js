...
function registerClaudeStatusline(options = {}) {
  ...
  if (!options.settingsPath && !hasClaudeSettingsDir(homeDir, options)) {
    if (!options.silent) console.log("AgentHalo: Claude Code settings not found - skipping statusline registration");
  ...
  if (existing && !existingIsOurs && !chainRequested) {
    if (!options.silent) console.log(`AgentHalo: existing Claude Code statusline detected at ${settingsPath} - leaving it in place`);
  ...
  if (existing && !existingIsOurs && chainRequested) {
    ...
    if (chainExplicitlyDisabled && chainedOriginal) {
      ...
      if (!options.silent) console.log(`AgentHalo: restored existing Claude Code statusline at ${settingsPath}`);
      return {
        installed: true,
        changed: true,
