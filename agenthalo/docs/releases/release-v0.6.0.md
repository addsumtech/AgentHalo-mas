...
- **Theme ecosystem** — fully pluggable theme system replacing hardcoded constants. Themes now define state-to-asset mapping, timings, hitboxes, sounds (#84, thanks @Rladmsrl), fade behavior, and capability metadata in `theme.json`; external themes load from the user config directory (`%APPDATA%/agenthalo/themes/` on Windows, `~/Library/Application Support/agenthalo/themes/` on macOS, `~/.config/agenthalo/themes/` on Linux) with mixed-format rendering (SVG + GIF + APNG + WebP); contributor tooling now includes a skeleton template, validation CLI (`scripts/validate-theme.js`), scaffold CLI, and capability badges
