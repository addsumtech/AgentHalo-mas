...
Pi 状态同步（global extension，state-only）：
  Pi 触发 session_start / before_agent_start / tool_call / tool_result / agent_end 等事件
    → ~/.pi/agent/extensions/agenthalo/index.ts（Pi extension runtime）
...
- macOS Apple Terminal 按会话 `pidChain` 的 TTY 精确选择标签页、恢复最小化并激活其窗口；不同会话共用 Terminal PID 时仍独立定位。选中与激活完成后才释放聚焦队列；无 TTY、标签页不存在或 Automation 被拒绝时保留应用级激活回退，并记录明确原因。
