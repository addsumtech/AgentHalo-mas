...
界面配色与角色主题分开：`uiColorTheme` 通过 settings controller 保存，预设在 `src/ui-color-theme.js`。`src/ui-color-theme-runtime.js` 只向设置、Dashboard 和任务 HUD 的本地窗口应用颜色变量，并在窗口加载和设置提交后更新。默认青苔绿保留已有用户外观；经典暖橘恢复旧版灰白与橘色风格。各配色跟随系统深浅模式，角色资产及工作、等待、警告等语义色不随配色改变。
