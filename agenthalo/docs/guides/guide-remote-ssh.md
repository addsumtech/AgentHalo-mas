...
Remote SSH** inside the AgentHalo app. `scripts/remote-deploy.sh` is intentionally
...
- A local AgentHalo instance is running
...
AgentHalo does not store SSH passwords or private-key passphrases. First-time host
...
3. If SSH needs first-time host-key confirmation, a passphrase, or an ssh-agent load, click **Authenticate**. AgentHalo opens your system terminal to run a plain `ssh` once.
4. Click **Deploy / Repair Hooks**, then connect the profile.
   - AgentHalo creates a dedicated local ingress for this profile and maintains an `ssh -R` reverse tunnel to that ingress
   ...
   - Then it copies hook files from the currently installed AgentHalo to the resolved remote runtime layout
...
**Copilot CLI** in **Settings → Agents** so AgentHalo accepts those remote hook
...
AgentHalo launches `~/.claude/hooks/codex-remote-monitor.js` as connection
...
AgentHalo inspects the effective local SSH configuration with `ssh -G` before it
connects. A `ProxyCommand` using exact `gh cs ssh ... --stdio` or
`gh codespace ssh ... --stdio` is automatically put in serialized mode. In
that mode AgentHalo never overlaps its own SSH/SCP children for the same
...
cannot be proven, AgentHalo stops and reports a recovery error instead of retrying
...
The `127.0.0.1:<port>` shown in Doctor is normal — it's the AgentHalo HTTP service
...
Remote Claude/Codex/Copilot hook
  ...
  -> Local AgentHalo state / permission handlers (canonical profile session ID)
...
also validates the nonce before state or permission data reaches AgentHalo.
...
the same Unix account. If AgentHalo finds an owner conflict, an ownerless legacy
deployment, or a live remote AgentHalo using that account's default config, it
...
separate Claude/Codex/Copilot user config roots, sessions, AgentHalo files, and
...
releasing its listening port. AgentHalo keeps the same port and retries a bounded
...
permission URL, and health probe are pinned to one exact port. AgentHalo blocks an
...
target; AgentHalo does not revive historical routing identity automatically.
...
profile-bound ingress rather than AgentHalo's general HTTP port. Use **Connect**
in the profile so AgentHalo can create and tear down that ingress together with
