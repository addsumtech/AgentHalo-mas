...
- `Windows native Codex + Windows AgentHalo`: official hooks로 기본 동작하며 JSONL fallback도 유지
- `WSL2 Codex + Windows AgentHalo + Linux 기본 home`: 기본값으로는 동작하지 않음. WSL 안에서 remote mode hooks를 설치하거나 remote fallback monitor를 실행해야 함
...
4. AgentHalo 자체를 확장해서 `\\wsl$\...` sessionDir를 직접 지원하게 만든다.
