...
Telegram Approval is an optional remote approval path for existing AgentHalo
permission bubbles. When a supported agent asks for tool permission, AgentHalo keeps
...
1. **Step 1 — Bot Token.** Create a dedicated bot with
   [@BotFather](https://t.me/botfather) using `/newbot`. Open AgentHalo Settings →
   **Remote Approval** → expand the **Telegram** card and paste the token into
   ...
   The token is stored outside `clawd-prefs.json` in AgentHalo's user-data
...
2. **Step 2 — Recipient.** Open [@userinfobot](https://t.me/userinfobot) in
   ...
   AgentHalo uses this one number both as the allowed approver (only this user can
...
3. **Step 3 — Enable & Verify.** Flip **Enable Telegram approval**.
   ...
   AgentHalo sends a standalone verification card. Tap either Allow or Deny in
...
- `409`: remove an existing webhook or stop the process on another machine,
  another AgentHalo profile, or another bot integration that polls the same token.
...
- Timeout: tap the standalone verification card within 60 seconds. If it was
  already tapped, also check the network or proxy because AgentHalo may not have
...
- If the desktop bubble resolves first, AgentHalo aborts the in-flight Telegram
  approval request.
- Repeated Telegram taps after a request is already handled do not resolve the
  permission twice.
- AgentHalo logs redact Telegram tokens, chat ids, and token-like values.

## Message Formatting

- Completion notifications render Assistant output through a conservative
  Markdown subset using Telegram-safe HTML. AgentHalo metadata such as the session
  title, agent, folder, and host is escaped as plain dynamic text rather than
  interpreted as Markdown.
- Approval, session-trust, and AskUserQuestion cards use AgentHalo-owned structure.
...
- Secret redaction runs before Markdown parsing. Unsupported HTML, unsafe link
  schemes, credentialed links, and image syntax degrade to visible text; AgentHalo
...
- If Telegram rejects the generated HTML as an entity-parse error, AgentHalo retries
...
Choose **Verify native and switch**. AgentHalo reuses the existing bot token,
...
If verification fails or times out, AgentHalo remains in the migration-required
...
other independent AgentHalo profile, wait a few seconds for Telegram to release
