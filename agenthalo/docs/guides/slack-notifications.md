...
Slack notifications are **one-way**. AgentHalo posts to a Slack channel when a
...
AgentHalo: in the desktop bubble, or in an interactive remote channel such as
Telegram or Feishu.
...
AgentHalo redacts recognisable secrets before sending — provider token prefixes,
...
allow" resolves it, AgentHalo stays silent — an auto-approved tool call must never
...
For a desktop request AgentHalo waits until the permission card has actually rendered
...
UI instead of being partially answerable in AgentHalo. For accepted payloads, Slack
...
permission request is dropped at the HTTP route, before AgentHalo ever announces it,
...
2. **Save it in AgentHalo.** **Settings → Remote Approval → Slack**, paste into
   step 1, then **Save**. For the bot transport also fill in the channel id.

   AgentHalo only accepts `https://hooks.slack.com/…`, compared by exact hostname —
...
Outside `clawd-prefs.json`, in `slack-notify.env` in AgentHalo's user-data
...
failure, which AgentHalo logs and does not retry.
...
Posting a completion does not acknowledge it in AgentHalo. A session that requires
...
- **No approvals from Slack.** Interactive buttons need Socket Mode or a public
  request URL. Until then Slack announces and AgentHalo decides.
