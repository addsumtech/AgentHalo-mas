...
3. AgentHalo 当前是否能自动感知“跑在 WSL 独立 Linux home 里的 Codex 会话”。

这三件事的答案分别不同。如果不拆开说，很容易把“官方支持 WSL2”和“AgentHalo 当前默认能不能识别 WSL 里的日志”误听成同一件事。

## TL;DR

- OpenAI 官方支持 Codex 跑在 WSL2 里。
- 当前 Codex CLI 通过 `[features].hooks = true` 开启 hooks；旧版本和部分文档使用的 `[features].codex_hooks` 已废弃。
- AgentHalo 当前以 Codex official hooks 为主，继续保留 `~/.codex/sessions` JSONL 轮询作为 fallback。
- 2026-04-26 本地已验证 Windows native Codex hooks 可用；Windows hook command 必须使用 PowerShell 的 `&` 调用形式。
- 当 AgentHalo 跑在 Windows、Codex 跑在 WSL 且仍使用 Linux 默认 home 时，AgentHalo 默认不会扫到 WSL 里的 `/home/<user>/.codex/sessions`。
- 所以“Codex 不支持 WSL”这个说法不准确。更准确的说法是：“Codex 官方支持 WSL2，但 Windows 上的 AgentHalo 不会自动修改或轮询 WSL 独立 Linux `~/.codex`；如果希望 WSL 内的 Codex 会话回传，需要在 WSL 里以 remote mode 安装 hooks，或共享 `CODEX_HOME`。”
...
AgentHalo 的 Codex installer 会写入 `~/.codex/hooks.json`，并在用户没有显式关闭 hooks 时开启这个 feature flag；如果看到已废弃的 `codex_hooks` key，会迁移到 `hooks`，同时保留用户显式设置的 false。
...
### 1. AgentHalo 以 official hooks 为主，JSONL 轮询兜底
...
- PermissionRequest 默认使用 intercept 模式：AgentHalo 会弹出真正的 Allow/Deny 气泡。用户可以把 Codex permission mode 切到 native，让 Codex AutoReview / 原生审批继续接管。
...
- 如果 AgentHalo 跑在 Windows，默认扫的是 `C:\Users\<user>\.codex\sessions`
- 如果 AgentHalo 跑在 Linux，默认扫的是 `/home/<user>/.codex/sessions`

因此，Windows 上运行的 AgentHalo 不会自动扫到 WSL Linux home 下的 `/home/<user>/.codex/sessions`。

### 2. AgentHalo 当前没有自动“安装到 WSL home”的逻辑
...
- `Codex native Windows + AgentHalo on Windows`：official hooks 开箱即用，JSONL 轮询兜底
- `Codex in WSL2 + AgentHalo on Windows + Linux 默认 home`：默认不通，除非在 WSL 里以 remote mode 安装 hooks，或启动远端 fallback monitor
- `Codex in WSL2 + 共享到 Windows 的 .codex`：按官方 `CODEX_HOME` 方案推导，可以共享配置和 session 状态

最后这一条是根据 OpenAI 官方的 `CODEX_HOME` 共享方案和 AgentHalo 当前 hook / fallback 轮询行为推导出的结论。

### 3. AgentHalo 仓库里有远程/旁路方案，但不是 WSL 开箱即用方案
...
它会在另一端轮询解析后的 `<CODEX_HOME>/sessions`，再把状态 POST 回本地 AgentHalo，
...
它说明仓库作者已经考虑过“日志在另一端、状态要回传”的场景，但这不等于“Windows 上的 AgentHalo 已经原生支持 WSL 里的 Codex 会话自动发现”。
...
3. AgentHalo 以 Codex official hooks 为主，JSONL 日志轮询为 fallback。
4. AgentHalo 当前同步 hooks 和 fallback 轮询的目标都是宿主机自己的 `.codex` home。
5. 所以当 Codex 运行在 WSL2 且仍使用 Linux 默认 `~/.codex` 时，Windows AgentHalo 默认不会自动发现这些会话。
...
> OpenAI 官方支持 Codex 跑在 WSL2 里。AgentHalo 现在通过 Codex official hooks 集成，并保留 JSONL 轮询作为 fallback。如果 AgentHalo 跑在 Windows、Codex 跑在 WSL 且使用 Linux 默认 home，AgentHalo 不会自动修改或轮询那份 WSL `~/.codex`；需要在 WSL 里以 remote mode 安装 hooks、共享 `CODEX_HOME`，或运行远端 fallback monitor。当前问题更像是 AgentHalo 的集成边界和文档没有讲清楚，而不是 Codex 官方不支持 WSL。

## 六、当前可行路径

如果目标是“让 Windows 上的 AgentHalo 感知 WSL 里的 Codex”，目前可考虑的路径有三类：

1. 按 OpenAI 官方文档，在 WSL 中把 `CODEX_HOME` 指到 Windows 的 `%USERPROFILE%\.codex`。
2. 在 WSL 里执行 `node hooks/codex-install.js --remote`，让 Codex official hooks 通过 WSL localhost 转发回 Windows AgentHalo。
3. 参考仓库现有的 [`hooks/codex-remote-monitor.js`](../../hooks/codex-remote-monitor.js) 思路，把 WSL 视作“另一端”，主动把状态回传给 Windows 上的 AgentHalo。
4. 修改 AgentHalo 自己的 Codex sessionDir 解析逻辑，显式支持 `\\wsl$\...` 路径。

其中：

- 第 1 条是 OpenAI 官方给出的共享方案。
- 第 2 条是 Codex hooks 可用时 AgentHalo 延迟最低的路径。
- 第 3 条是 official hooks 不可用或被禁用时的 fallback。
- 第 4 条属于 AgentHalo 侧新增功能，不是当前默认行为。
