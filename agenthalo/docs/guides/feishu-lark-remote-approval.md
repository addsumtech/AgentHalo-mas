...
Feishu / Lark approval is an optional remote approval path for existing AgentHalo
...
AgentHalo ships a single approval channel for both. Pick your platform in
...
- **No custom domains.** Only the two official platforms above can be selected.
  Your App Secret travels on these requests, so AgentHalo will not send it to a host
  you type in.
- **No auto-detection.** Self-built App IDs start with `cli_` on *both*
  ...
  turn would mean sending your App Secret to a platform you do not use, so AgentHalo
...
1. **Step 1 — Platform & App Credentials.** Choose Feishu or Lark, then create a
   ...
   **App ID** and **App Secret**. Verification Token and Encrypt Key are optional
   ...
   in AgentHalo's user-data directory, with `0600` permissions where the OS supports
   it.
...
> derived from AgentHalo's actual API calls and the official docs — it has **not**
...
AgentHalo does **not** call CardKit entity, card template, message-read, or
...
Cards follow AgentHalo's current language setting (English, 简体中文, 繁體中文,
한국어, 日本語, Português (Brasil), Español) — titles, field labels, buttons, question steps, result status,
and the source label. Switching AgentHalo's language affects cards sent afterwards
...
- DND does not auto-decide. See the DND behavior described in
  ...
  AgentHalo never makes the permission decision for you.
