...
AgentHalo can keep private, local footprints of the agent activity it actually receives. Open **Settings → Footprints**, or choose **Open Footprints** from AgentHalo's right-click menu.
...
Footprints counts accepted AgentHalo activity signals. It never guesses work time from process uptime or idle thresholds.
...
Agent integrations do not expose identical signals. A dash (`—`) means that AgentHalo cannot reliably measure that value for that agent. It is intentionally different from `0`, which means a supported metric was observed and its count was actually zero. Registered custom HTTP agents are not included in Footprints v1 because their event vocabulary is user-defined and has no trusted metric mapping.
...
A quiet period is meaningful only while AgentHalo was able to record. Footprints therefore stores coverage separately from activity:

- a covered empty cell means AgentHalo was running and recording, but received no accepted signal;
- an uncovered period means AgentHalo was closed, the computer was suspended, or Footprints recording was turned off;
- dates before Footprints was first enabled are marked as not started and are never backfilled;
- Do Not Disturb still records accepted activity. DND suppresses interruption and animation; it does not pretend that AgentHalo stopped observing;
...
The minimized tickets contain a timestamp, frozen local time fields, built-in agent id, local/WSL/remote scope class, metric flags, and HMAC identifiers used for short-term deduplication. AgentHalo never writes prompt or response text, commands, tool names or arguments, tool output, paths, filenames, project or repository names, branches, session titles, cwd, account identifiers, raw session/profile/event/turn/tool identifiers, or the original upstream event name into recap storage.
...
If AgentHalo recovered damaged preferences or could not safely use them, Footprints stays paused even when the saved recording preference is on. The page explains the pause. Review Settings and use the recording switch off/on to resume when Settings writes are available; unreadable or backup-failed preferences must be repaired and AgentHalo restarted first.
