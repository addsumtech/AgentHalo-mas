...
大多数 agent 生命周期事件会映射到同一组 AgentHalo 状态。

Subagent 事件仍映射到逻辑 `juggling` 状态，但 AgentHalo 主题现在会按 live 子代理数量选择分层素材：1 个子代理使用 `clawd-headphones-groove.svg`，2 个以上使用 `clawd-working-juggling.svg`。旧版 AgentHalo conducting 素材已退役；Calico 和云宝的 2+ 子代理分层仍使用各自的 conducting 动画。
...
Kimi Code CLI（Kimi-CLI）现已采用 hook-only 集成（`~/.kimi/config.toml`），下面这 13 个 hook 事件会映射到 AgentHalo 的共享状态：
...
`PermissionRequest` 自 Phase 2 起是阻塞式权限审批：hook 等待 AgentHalo 本地气泡或远程审批产生人工决定，并通过 stdout 的 `hookSpecificOutput` 回答 allow/deny。在完成 ZCode 工具面与会话身份审计前，权限自动化会 defer。上表的 `notification` 映射只在 fail-closed 路径（tool name 缺失 / unknown）或 AgentHalo 未运行时触发；真正的决定不会 POST `/state`。当前集成没有 ZCode `SessionEnd` 事件，会话完成依赖 `Stop` 和 AgentHalo 原有的进程存活 / stale session 清理。当 AgentHalo 无决定时（超时、断连、DND、气泡关闭），hook 输出 `{}`，由 ZCode 原生权限流程接管。

## Pi Extension 事件

Pi 使用全局 extension（`~/.pi/agent/extensions/clawd-on-desk`），会把交互式会话生命周期事件映射到 AgentHalo 的共享状态：

| Pi Extension Event | AgentHalo Event | 状态 |
|---|---|---|
...
Pi 当前在 AgentHalo 中是 state-only 集成：AgentHalo 不接管权限、不新增确认弹窗，Pi 保持默认 YOLO 执行行为。

## 极简模式

拖到屏幕右边缘（或右键 →"极简模式"）进入——半身露出在屏幕边缘，悬停时探出来。

| 触发 | 极简反应 | AgentHalo | Calico | 云宝 |
|---|---|---|---|---|
...
彩蛋——试试双击、连点 4 下、或反复戳 AgentHalo，会有隐藏反应。
