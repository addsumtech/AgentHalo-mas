...
3. Whether AgentHalo can currently auto-detect Codex sessions that live inside WSL's separate Linux home.

Those answers are not the same. Most confusion comes from mixing the official Codex product support story with AgentHalo's current integration boundaries.
...
- AgentHalo now uses Codex official hooks as the primary integration and keeps `~/.codex/sessions` JSONL polling as fallback.
...
- When AgentHalo runs on Windows while Codex runs inside WSL with the default Linux home directory, AgentHalo does not automatically see `/home/<user>/.codex/sessions`.
- So "Codex does not support WSL" is inaccurate. The more accurate statement is: "Codex officially supports WSL2, but Windows AgentHalo does not automatically modify or poll WSL's separate Linux `~/.codex`; install hooks in WSL remote mode or share `CODEX_HOME` when you want WSL Codex sessions to report back."
...
AgentHalo's Codex installer writes `~/.codex/hooks.json` and enables this feature flag unless the user explicitly set hooks to `false`. If it sees the deprecated `codex_hooks` key, it migrates it to `hooks` while preserving an explicit false value.
...
## 2. AgentHalo's current implementation

### AgentHalo uses official hooks first, with JSONL polling as fallback
...
- PermissionRequest defaults to intercept mode: AgentHalo shows a real Allow/Deny bubble. Users can switch Codex permission mode to native if they want Codex AutoReview/native prompts to stay in charge.
...
- if AgentHalo runs on Windows, it polls `C:\Users\<user>\.codex\sessions`
- if AgentHalo runs on Linux, it polls `/home/<user>/.codex/sessions`

So a Windows-hosted AgentHalo does not automatically poll `/home/<user>/.codex/sessions` inside WSL.

### AgentHalo currently has no automatic "install into WSL home" path
...
- `Codex native on Windows + AgentHalo on Windows`: works out of the box through official hooks, with JSONL fallback
- `Codex in WSL2 + AgentHalo on Windows + default Linux home`: does not work by default unless hooks are installed inside WSL in remote mode or a remote-side monitor is started
...
The last point is an inference from OpenAI's documented `CODEX_HOME` sharing flow plus AgentHalo's current hook and fallback polling behavior.

### AgentHalo has remote-side hooks and a monitor fallback, but not WSL auto-discovery
...
3. AgentHalo uses official Codex hooks as primary integration and JSONL polling as fallback.
4. AgentHalo currently syncs hooks and polls fallback logs in the host machine's own `.codex` home.
5. So if Codex runs inside WSL2 and still uses Linux `~/.codex`, Windows AgentHalo does not auto-detect those sessions by default.
...
> OpenAI officially supports running Codex in WSL2. AgentHalo integrates with Codex through official hooks, with JSONL polling kept as fallback. If AgentHalo runs on Windows while Codex runs in WSL with Linux's default home directory, AgentHalo will not automatically modify or poll that WSL `~/.codex`; install hooks in WSL remote mode, share `CODEX_HOME`, or run the remote fallback monitor. The current issue is better described as a AgentHalo integration and documentation boundary, not as OpenAI failing to support Codex on WSL.

## 6. Current workable paths

If the goal is "make Windows AgentHalo react to Codex running in WSL", the realistic paths today are:

1. Use OpenAI's documented sharing approach and point WSL `CODEX_HOME` at the Windows `%USERPROFILE%\.codex`.
2. Install Codex official hooks inside WSL with `node hooks/codex-install.js --remote`, so WSL Codex POSTs back to Windows AgentHalo through WSL localhost forwarding.
...
4. Extend AgentHalo itself to support an explicit `\\wsl$\...` session directory.

Those options are not equivalent:

- option 1 is an OpenAI-documented sharing flow
- option 2 is the lowest-latency AgentHalo path when Codex hooks are available inside WSL
- option 3 is the fallback for hook-disabled Codex sessions
- option 4 would be a new AgentHalo feature, not current default behavior
