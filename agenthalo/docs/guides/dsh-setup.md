...
AgentHalo's first DeepSeek Harness (DSH) integration is experimental and supports the
DSH `web` profile. A AgentHalo-managed plugin runs inside DSH and uses public APIs
for both state observation and ordinary blocking approvals. AgentHalo does not read
...
preview. AgentHalo keeps an explicit table of verified DSH releases, each bound to its
...
Unlisted versions fail before AgentHalo changes the DSH profile.

## Behavior

The plugin observes `session/created`, `session/event`, and `session/disposed`.
It sends a minimal allowlisted payload to AgentHalo's dynamically discovered local
...
| DSH public event | AgentHalo event / state |
| --- | --- |
...
- AgentHalo **Allow** returns DSH `allowed-once`.
- AgentHalo **Deny** returns DSH `rejected`.
- HTTP 204, timeout, invalid response, DND, disabled integration, or unavailable
  AgentHalo calls `next()` so DSH's native web answerer remains authoritative.
- DSH cancellation aborts the pending HTTP request.
- `policy="never"` is enforced by DSH before listener dispatch and cannot be
  overridden by AgentHalo.

`ask_user_question` stays entirely in DSH's native provider. AgentHalo does not
replace private provider state, create a second question bubble, or auto-answer
questions. DSH approvals also remain manual when AgentHalo auto-tools or unattended
...
`DSH_HOME` is honored when it is a trimmed non-empty value; otherwise AgentHalo uses
`~/.dsh`.

## Install and repair

Open **Settings → Agents**, find **DeepSeek Harness (web, experimental)**, and
click **Install**. Install succeeds only after AgentHalo has:
...
3. verified both DSH profile rows, the final profile-local package resolution,
   the AgentHalo ownership marker, protocol, compatibility range, and bundle hash.
...
If DSH is only used through `npx`, AgentHalo does not download it automatically.
...
CLI. The generation is protected by a AgentHalo-owned manual reference until it is
...
AgentHalo automatically recovers a stranded lock only when its owner metadata is
...
the lock, close every AgentHalo instance using that DSH home, verify the reported PID
...
for inspection; AgentHalo never recursively removes a canonical lock during owner
...
AgentHalo verifies ownership before it calls the official remove command. A user
...
the dependency row, bundle row, and resolved AgentHalo package are all gone.

`$DSH_HOME/profiles/node_modules` is DSH/pnpm's shared dependency fallback, not a
AgentHalo ownership anchor or cleanup target. AgentHalo may report what resolves there,
...
warnings, and rely on DSH's native web flow whenever AgentHalo yields no decision.
...
- On 2026-08-29, **rc.6 source-checkout** runs on Windows x64 and macOS
  verified real API-backed DSH web sessions plus manual AgentHalo **Allow Once**
...
- Closing the local bubble does not deny the request. If a configured Telegram
  ...
  receives no AgentHalo decision and continues its native flow.
