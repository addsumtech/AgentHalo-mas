...
远程 SSH 部署唯一受支持的入口是 AgentHalo 应用内的 **Settings -> 远程 SSH**。
...
AgentHalo 不保存 SSH 密码或私钥口令。首次 host key 确认、passphrase、ssh-agent
加载都交给系统 `ssh` 和系统终端处理。
...
3. 如果 SSH 需要首次确认 host key、输入 passphrase 或加载 ssh-agent，点 **首次认证**。AgentHalo 会打开系统终端运行一次普通 `ssh`。
4. 点击 **部署 / 修复 Hook**，然后连接 profile。
   - AgentHalo 会为该 profile 创建专属本地入口，并维护指向它的 `ssh -R` 反向隧道
   - AgentHalo 会原子写入 profile 身份，把 hook 和 Claude 静态权限 URL pin 到精确远端端口；隧道不会接触通用 `/state`、`/permission` 入口
   - 然后从当前已安装的 AgentHalo 应用复制 hook 文件到解析出的远端 runtime layout
...
打开 **Copilot CLI**，这样 AgentHalo 才会接收远程 hook 事件；不需要点
**Install / 安装**，除非你也想在本机安装 Copilot hooks。

如果配置里开启了 **连接时自动启动 Codex 兜底监控**，AgentHalo 会把
...
AgentHalo 会在连接前用 `ssh -G` 检查本机实际生效的 SSH 配置。`ProxyCommand` 精确使用
`gh cs ssh ... --stdio` 或 `gh codespace ssh ... --stdio` 时会自动进入 serialized
模式。同一 Codespace 上，AgentHalo 自己管理的 SSH/SCP 不会重叠：Node 探测和 monitor
维护先完成并关闭，连接就绪检查则放在唯一的持久 `ssh -R` session 里。

执行 **部署 / 修复 Hook**、断开、清理或身份 / runtime 变更时，AgentHalo 会通过 stdin
EOF 请求持久 readiness 进程自然退出，并在开始下一条 SSH 操作前等待外层 child
`close`。如果无法证明 transport 已排空，AgentHalo 会停止并显示恢复错误，不会自动重放
...
AgentHalo HTTP 服务，不是远端集群的 IP。远端 hook 也不直接访问你的局域网 IP。
...
远端 Claude/Codex/Copilot hook
  ...
  -> 本机 AgentHalo 状态 / 权限处理（profile canonical session ID）
...
一个 pin 住的端口，不再扫描 `23333-23337`；接收端在状态或权限数据进入 AgentHalo
...
冲突、无属主旧部署或远端本地 AgentHalo 正在使用账号默认配置时，AgentHalo 会在写入前阻止。
...
Claude/Codex/Copilot 用户配置、sessions、AgentHalo 文件和启动 wrapper；真实 SSH/CLI
...
AgentHalo 会保留原端口，按退避节奏最多重试四次（目前总计约三分钟）。界面显示
...
和健康检查都 pin 在一个精确端口上。当前目标重新部署前，AgentHalo 会阻止普通连接。

历史部署记录只用于所有权清理，不代表还有一个可直接恢复的 active 端口槽。
即使把端口从 `23333 → 23334 → 23333` 改回去，也必须为当前目标重新部署；AgentHalo
...
安全版 Remote SSH 不支持这样做。本地目标是临时的 profile 专属入口，不是 AgentHalo
通用 HTTP 端口。请使用 profile 的 **连接**，让 AgentHalo 一起创建和销毁入口与隧道。
