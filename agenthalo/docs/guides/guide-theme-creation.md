...
Install a downloaded AgentHalo theme, or create your own desktop pet theme with custom characters and animations.

## Install A Downloaded Theme

A AgentHalo theme is a folder whose top level contains `theme.json`. The folder name is the theme id; the display name shown in AgentHalo comes from `theme.json.name`.
...
3. Put the theme folder in your AgentHalo user themes directory:
   - Windows: `%APPDATA%/agenthalo/themes/pixel-cat/`
   - macOS: `~/Library/Application Support/agenthalo/themes/pixel-cat/`
   - Linux: `~/.config/agenthalo/themes/pixel-cat/`

4. Open `Settings...` -> `Theme` and select the theme. If AgentHalo was already open and the theme does not appear, restart AgentHalo.
...
1. Scaffold a theme:
   ...
   ```
   The script writes to your AgentHalo user themes directory by default:
   - Windows: `%APPDATA%/agenthalo/themes/my-theme/`
   - macOS: `~/Library/Application Support/agenthalo/themes/my-theme/`
   - Linux: `~/.config/agenthalo/themes/my-theme/`
...
5. Open `Settings...` -> `Theme` and select your theme. Restart AgentHalo if the new folder is not listed yet.

6. (Optional) Validate:
   ```bash
   node scripts/validate-theme.js ~/.config/agenthalo/themes/my-theme
   ```
...
Do not build a user theme that depends on JavaScript inside SVG files. The built-in Cloudling theme uses `trustedRuntime.scriptedSvgFiles`, but that capability is only honored for themes loaded from AgentHalo's packaged/repo `themes/` directory. If an external theme declares `trustedRuntime`, AgentHalo ignores it.
...
| `customization.accessories` | Opts the theme into AgentHalo's built-in **head** accessory catalog only when every reachable visual has a deterministic attachment or an explicit hidden policy. Exact `files` entries may also prepare optional assets exposed by the animation-override picker; those files become required theme assets. |
...
- Head attachments may define sparse `itemOverrides.<catalog-id>.files` entries. An exact item/file descriptor wins over the materialized base file descriptor; there is no additional default or mini fallback. `itemOverrides` requires the PR #811 accessory runtime (the first release after 0.16.0); AgentHalo 0.16.0 and older reject this new key instead of ignoring it.
...
`mini-working` is optional. If you provide `miniMode.states["mini-working"]`, AgentHalo can show a compact working animation while the pet is in mini mode. If you omit it, working/thinking/juggling events do not break mini mode; AgentHalo keeps the current mini visual.
...
If two themes have very different visible body heights even though the window size is the same, add a `layout` block. This lets AgentHalo align the character by visible body area and baseline instead of the raw file canvas:
...
2. Users extract to `{userData}/themes/`
   - Windows: `%APPDATA%/agenthalo/themes/`
   - macOS: `~/Library/Application Support/agenthalo/themes/`
   - Linux: `~/.config/agenthalo/themes/`
...
4. Restart AgentHalo only if a newly copied theme folder does not appear yet.
