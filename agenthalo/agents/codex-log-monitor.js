...
class CodexLogMonitor {
  ...
  constructor(agentConfig, onStateChange, options = {}) {
    ...
    this._archivedDir = options.archivedDir
      || path.join(this._codexDir || path.dirname(this._baseDir), "archived_sessions");
    this._archivedSessionIds = new Set();
    this._archivedDirStamp = null;
    this._onSessionClosed = typeof options.onSessionClosed === "function" ? options.onSessionClosed : null;
    this._onSessionArchived = typeof options.onSessionArchived === "function"
  ...
  stop() {
    ...
    this._startupRecoveryFilesScanned = 0;
    this._startupRecoveryBytesScanned = 0;
    this._archivedSessionIds.clear();
    this._archivedDirStamp = null;
  }

  isSessionArchived(sessionId) {
    return this._archivedSessionIds.has(String(sessionId || "").replace(/^codex:/, ""));
  }

  _refreshArchivedSessions() {
    let names;
    let stamp;
    try {
      const stat = fs.statSync(this._archivedDir);
      stamp = `${stat.dev}:${stat.ino}:${stat.mtimeMs}:${stat.ctimeMs}`;
      if (stamp === this._archivedDirStamp) return;
      names = fs.readdirSync(this._archivedDir);
    } catch (err) {
      // A transient read failure is not evidence that tasks were unarchived.
      if (err.code !== "ENOENT") return;
      stamp = "missing";
      names = [];
    }
    const archived = new Set(names
      .filter((name) => name.startsWith("rollout-") && name.endsWith(".jsonl"))
      .map((name) => this._extractSessionId(name))
      .filter(Boolean));
    const newlyArchived = [...archived].filter((id) => !this._archivedSessionIds.has(id));
    this._archivedSessionIds = archived;
    this._archivedDirStamp = stamp;

    for (const [filePath, tracked] of this._tracked) {
      if (!this.isSessionArchived(tracked.sessionId)) continue;
      this._clearPendingUserInputsForTrackedSession(tracked, "session-archived");
      tracked.lastState = "idle";
      tracked.activeTurnId = null;
      tracked.turnBoundaryOpen = false;
      this._replayWork.delete(filePath);
      // Keep the committed read offset and metadata for an eventual unarchive;
      // restoring the same rollout must not replay the completed conversation.
      this._retireTrackedFile(filePath, tracked);
    }
    for (const pending of [this._replayWork, this._deferredRecent,
      this._deferredBackground, this._startupRecoveryCandidates]) {
      for (const filePath of pending.keys()) {
        if (this.isSessionArchived(this._extractSessionId(path.basename(filePath)))) {
    ...
    for (const id of newlyArchived) {
      ...
      if (this._classifier && typeof this._classifier.clear === "function") {
        this._classifier.clear(sessionId);
      }
      if (this._onSessionArchived) this._onSessionArchived(sessionId);
    }
  }
  ...
  _poll() {
    ...
    this._reconcileExitedExecSessions();
  ...
  _recoverStalePendingUserInput(filePath, fileName, recoveryStat = null, options = {}) {
    if (this.isSessionArchived(this._extractSessionId(fileName))) return null;
  ...
  _pollFile(filePath, fileName, pollContext = null) {
    if (this.isSessionArchived(this._extractSessionId(fileName))) {
      return { kind: "ignored", requestedBytes: 0, bytesRead: 0 };
    }
  ...
  _reconcileExitedExecSessions() {
    if (this._platform !== "darwin" || !this._onSessionClosed) return;
    const now = Date.now();
    for (const [filePath, tracked] of this._tracked) {
      if (!tracked.hasEmittedState || tracked.execEnded || this._replayWork.has(filePath)
        || (tracked.codexSource !== "exec" && tracked.codexOriginator !== "codex_exec")) continue;
      let stat;
      try { stat = fs.statSync(filePath); } catch { continue; }
      // Silence alone cannot end a long tool/model call. Require an unchanged
      // file and two successful process inventories with no rollout owner.
      if (now - stat.mtimeMs < 30000) { tracked.execMissingScan = null; continue; }
      const pid = this._findCodexWriterPid(filePath);
      if (!this._writerPidScanValid || pid) { tracked.execMissingScan = null; continue; }
      const fingerprint = `${stat.size}:${stat.mtimeMs}`;
      const previous = tracked.execMissingScan;
      tracked.execMissingScan = { fingerprint, scanAt: this._writerPidCacheAt };
      if (!previous || previous.fingerprint !== fingerprint
        || this._writerPidCacheAt - previous.scanAt < 5000) continue;
      tracked.execEnded = true;
      tracked.lastState = "idle";
      tracked.activeTurnId = null;
      tracked.turnBoundaryOpen = false;
      this._clearPendingUserInputsForTrackedSession(tracked, "exec-exited");
      this._onSessionClosed(tracked.sessionId, "exec-exited");
    }
  }
  ...
  _emitStateChange(tracked, state, event, extra = null) {
    tracked.execEnded = false;
    tracked.execMissingScan = null;
    tracked.lastState = state;
